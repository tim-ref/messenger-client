...
class Api {
  ...
  Future<Uri?> getAvatarUrl(String userId) async {
    final requestUri = Uri(
      path: '_matrix/client/v3/profile/${Uri.encodeComponent(userId)}/avatar_url',
    );
    final request = Request('GET', baseUri!.resolveUri(requestUri));
    if (bearerToken != null) {
      request.headers['authorization'] = 'Bearer ${bearerToken!}';
    ...
    final responseString = utf8.decode(responseBody);
    final json = jsonDecode(responseString);
    return ((v) => v != null ? Uri.parse(v as String) : null)(json['avatar_url']);
  }
  ...
  Future<String?> getDisplayName(String userId) async {
    final requestUri = Uri(
      path: '_matrix/client/v3/profile/${Uri.encodeComponent(userId)}/displayname',
    );
    final request = Request('GET', baseUri!.resolveUri(requestUri));
    if (bearerToken != null) {
      request.headers['authorization'] = 'Bearer ${bearerToken!}';
  ...
  Future<void> sendToDevice(
    ...
    request.bodyBytes = utf8.encode(
      jsonEncode({
        'messages': messages.map((k, v) => MapEntry(k, v.map((k, v) => MapEntry(k, v)))),
      }),
    );
    final response = await httpClient.send(request);
    final responseBody = await response.stream.toBytes();
