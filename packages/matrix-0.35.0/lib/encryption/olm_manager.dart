...
class OlmManager {
  ...
  String? get pickledOlmAccount => enabled ? _olmAccount!.pickle(client.userID!) : null;
    ...
    if (olmAccount == null) {
      try {
        ...
        if (!await uploadKeys(
          ...
          dehydratedDevicePickleKey: dehydratedDeviceAlgorithm != null ? pickleKey : null,
        )) {
    ...
    try {
      ...
      if (oldKeyCount != null) {
        ...
        final oldOTKsNeedingUpload =
            json.decode(olmAccount.one_time_keys())['curve25519'].entries.length as int;
        // generate one-time keys
        // we generate 2/3rds of max, so that other keys people may still have can
        // still be used
        final oneTimeKeysCount = (olmAccount.max_number_of_one_time_keys() * 2 / 3).floor() -
            oldKeyCount -
      ...
      if (uploadDeviceKeys) {
        final Map<String, dynamic> keys = json.decode(olmAccount.identity_keys());
      ...
      for (final entry in json.decode(olmAccount.one_time_keys())['curve25519'].entries) {
        final key = entry.key;
        final value = entry.value;
      ...
      if (signedFallbackKeys.isEmpty && signedOneTimeKeys.isEmpty && !uploadDeviceKeys) {
      ...
      if (ourDeviceId != client.deviceID) {
        if (dehydratedDeviceAlgorithm == null || dehydratedDevicePickleKey == null) {
          throw Exception(
        ...
        await client.uploadDehydratedDevice(
          ...
          deviceKeys: uploadDeviceKeys ? MatrixDeviceKeys.fromJson(deviceKeys) : null,
          oneTimeKeys: signedOneTimeKeys,
          fallbackKeys: signedFallbackKeys,
          deviceData: {
            'algorithm': dehydratedDeviceAlgorithm,
            'device': encryption.olmManager.pickleOlmAccountWithKey(dehydratedDevicePickleKey),
          },
        );
        return true;
      }
      ...
      if (!uploadDeviceKeys &&
        ...
        for (final otk in signedOneTimeKeys.values) {
          ...
          try {
            final String identity = json.decode(olmAccount.identity_keys())['curve25519'];
  ...
  Future<void> handleDeviceOneTimeKeysCount(
    ...
    await _otkUpdateDedup.fetch(
      () => runBenchmarked('handleOtkUpdate', () async {
        ...
        if (keyCount < (_olmAccount!.max_number_of_one_time_keys() / 2) || !unusedFallbackKey) {
          await uploadKeys(
            oldKeyCount:
                keyCount < (_olmAccount!.max_number_of_one_time_keys() / 2) ? keyCount : null,
  ...
  Future<void> storeOlmSession(OlmSession session) async {
    ...
    final ix =
        _olmSessions[session.identityKey]!.indexWhere((s) => s.sessionId == session.sessionId);
    if (ix == -1) {
      // add a new session
  ...
  Future<ToDeviceEvent> _decryptToDeviceEvent(ToDeviceEvent event) async {
    ...
    if (content.ciphertextOlm == null || !content.ciphertextOlm!.containsKey(identityKey)) {
  ...
  Future<List<OlmSession>> getOlmSessionsFromDatabase(String senderKey) async {
    final olmSessions = await encryption.olmDatabase?.getOlmSessions(senderKey, client.userID!);
  ...
  Future<List<OlmSession>> getOlmSessions(
    ...
    sess.sort(
      (a, b) => a.lastReceived == b.lastReceived
          ...
          : (b.lastReceived ?? DateTime(0)).compareTo(a.lastReceived ?? DateTime(0)),
    );
    return sess;
  }
  ...
  Future<void> restoreOlmSession(String userId, String senderKey) async {
    ...
    if (_restoredOlmSessionsTime.containsKey(mapKey) &&
        DateTime.now().subtract(Duration(hours: 1)).isBefore(_restoredOlmSessionsTime[mapKey]!)) {
      Logs().w(
  ...
  Future<void> startOutgoingOlmSessions(List<DeviceKeys> deviceKeys) async {
    ...
    for (final userKeysEntry in response.oneTimeKeys.entries) {
      ...
      for (final deviceKeysEntry in userKeysEntry.value.entries) {
        ...
        final fingerprintKey = client.userDeviceKeys[userId]!.deviceKeys[deviceId]!.ed25519Key;
        final identityKey = client.userDeviceKeys[userId]!.deviceKeys[deviceId]!.curve25519Key;
          ...
          try {
            ...
            await storeOlmSession(
              OlmSession(
                ...
                session: session,
                lastReceived: DateTime.now(), // we want to use a newly created session
              ),
            );
          } catch (e, s) {
            session.free();
            Logs().e('[LibOlm] Could not create new outbound olm session', e, s);
          }
        }
      }
    }
  ...
  Future<Map<String, dynamic>> encryptToDeviceMessagePayload(
    ...
    final sess = await getOlmSessions(device.curve25519Key!, getFromDb: getFromDb);
    if (sess.isEmpty) {
  ...
  Future<Map<String, Map<String, Map<String, dynamic>>>> encryptToDeviceMessage(
    ...
    deviceKeysWithoutSession.removeWhere(
      (DeviceKeys deviceKeys) => olmSessions[deviceKeys.curve25519Key]?.isNotEmpty ?? false,
    );
...
class NoOlmSessionFoundException implements Exception {
  ...
  @override
  String toString() => 'No olm session found for ${device.userId}:${device.deviceId}';
}
