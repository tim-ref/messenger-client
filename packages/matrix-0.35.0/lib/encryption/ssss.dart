...
class SSSS {
  ...
  final CachedStreamController<String> onSecretStored = CachedStreamController();
  ...
  static DerivedKeys deriveKeys(Uint8List key, String name) {
    ...
    b[0] = 2;
    final hmacKey = Hmac(sha256, prk.bytes).convert(aesKey.bytes + utf8.encode(name) + b);
    return DerivedKeys(
  ...
  static Future<String> decryptAes(
    ...
    final decipher = await uc.aesCtr.encrypt(cipher, keys.aesKey, base64decodeUnpadded(data.iv));
  ...
  String? get defaultKeyId => client
      .accountData[EventTypes.SecretStorageDefaultKey]?.parsedSecretStorageDefaultKeyContent.key;
  ...
  SecretStorageKeyContent? getKey(String keyId) {
    return client.accountData[EventTypes.secretStorageKey(keyId)]?.parsedSecretStorageKeyContent;
  }
  ...
  Future<String> getStored(String type, String keyId, Uint8List key) async {
    ...
    final encryptedContent = secretInfo.content.tryGetMap<String, Object?>('encrypted');
  ...
  Future<void> validateAndStripOtherKeys(
    ...
    final otherKeys = Set<String>.from(encryptedContent.keys.where((k) => k != keyId));
    ...
    if (cacheTypes.contains(type)) {
      // cache the thing
      final ciphertext =
          encryptedContent.tryGetMap<String, Object?>(keyId)?.tryGet<String>('ciphertext');
      if (ciphertext == null) {
  ...
  Future<void> request(String type, [List<DeviceKeys>? devices]) async {
    ...
    devices.removeWhere(
      (DeviceKeys d) =>
          d.userId != client.userID || !d.verified || d.blocked || d.deviceId == client.deviceID,
    );
    if (devices.isEmpty) {
  ...
  Future<void> periodicallyRequestMissingCache() async {
    if (_isPeriodicallyRequestingMissingCache ||
        (_lastCacheRequest != null &&
            DateTime.now().subtract(Duration(minutes: 15)).isBefore(_lastCacheRequest!)) ||
  ...
  Future<void> handleToDeviceEvent(ToDeviceEvent event) async {
    if (event.type == EventTypes.SecretRequest) {
      ...
      if (event.sender != client.userID || !client.userDeviceKeys.containsKey(client.userID)) {
      ...
      final device =
          client.userDeviceKeys[client.userID]!.deviceKeys[event.content['requesting_device_id']];
      ...
      final secret = await getCached(type);
      if (secret == null) {
        Logs().i('[SSSS] We don\'t have the secret for $type ourself, ignoring');
      ...
      if (db != null) {
        ...
        if (keyId != null) {
          final ciphertext =
              (client.accountData[request.type]!.content.tryGetMap<String, Object?>('encrypted'))
  ...
  Set<String>? keyIdsFromType(String type) {
    ...
    final contentEncrypted = data.content.tryGetMap<String, Object?>('encrypted');
...
class OpenSSSS {
  ...
  String? get recoveryKey => isUnlocked ? SSSS.encodeRecoveryKey(privateKey!) : null;
  ...
  Future<void> _postUnlock() async {
    ...
    if (ssss.encryption.crossSigning.enabled &&
        ...
        (ssss.keyIdsFromType(EventTypes.CrossSigningMasterKey)?.contains(keyId) ?? false) &&
        (ssss.client.isUnknownSession ||
            ssss.client.userDeviceKeys[ssss.client.userID]!.masterKey?.directVerified != true)) {
      try {
