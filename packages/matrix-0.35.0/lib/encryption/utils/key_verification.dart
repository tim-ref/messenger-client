...
/*
 * Modified by akquinet GmbH on 2025-01-30
 * Originally forked from https://pub.dev/api/archives/matrix-0.33.0.tar.gz
 */
...
List<String> _calculatePossibleMethods(
  ...
  copyKnownMethods.removeWhere((element) => !copyPayloadMethods.contains(element));
  ...
  if (isQrSupported(knownMethods, payloadMethods)) {
    ...
    if (payloadMethods.contains(EventTypes.QRScan) && knownMethods.contains(EventTypes.QRShow)) {
      output.add(EventTypes.QRShow);
    }
    if (payloadMethods.contains(EventTypes.QRShow) && knownMethods.contains(EventTypes.QRScan)) {
...
class KeyVerification {
  ...
  bool get isDone =>
      canceled || {KeyVerificationState.error, KeyVerificationState.done}.contains(state);
  ...
  List<String> get knownVerificationMethods {
    ...
    if (client.verificationMethods.contains(KeyVerificationMethod.qrShow) && qrCanWork) {
      methods.add(EventTypes.QRShow);
      methods.add(EventTypes.Reciprocate);
    }
    if (client.verificationMethods.contains(KeyVerificationMethod.qrScan) && qrCanWork) {
  ...
  QRMode getOurQRMode() {
    ...
    if (client.userID == userId) {
      if (client.encryption != null &&
          ...
          (client.userDeviceKeys[client.userID]?.masterKey?.directVerified ?? false)) {
        ...
        case EventTypes.KeyVerificationReady:
          ...
          if (!isQrSupported(knownVerificationMethods, payload['methods'])) {
            if (knownVerificationMethods.contains(EventTypes.Sas)) {
              final method = _method = _makeVerificationMethod(possibleMethods.first, this);
              await method.sendStart();
              setState(KeyVerificationState.waitingAccept);
            }
          } else {
          ...
          if (_method != null) {
            ...
            if (payload['method'] == _method!.type) {
              ...
              if (entries.first == ourEntry) {
                ...
                thisLastStep =
                    lastStep = EventTypes.KeyVerificationRequest; // we fake the last step
  ...
  Future<void> acceptVerification() async {
    ...
    setState(KeyVerificationState.waitingAccept);
    if (lastStep == EventTypes.KeyVerificationRequest) {
      final copyKnownVerificationMethods = List<String>.from(knownVerificationMethods);
      ...
      if (userId == client.userID) {
        if (!(client.userDeviceKeys[client.userID]?.deviceKeys[deviceId]
                ...
                false) &&
            !(client.userDeviceKeys[client.userID]?.masterKey?.verified ?? false)) {
          copyKnownVerificationMethods.removeWhere((element) => element.startsWith('m.qr_code'));
      ...
      await _method!.handlePayload(EventTypes.KeyVerificationStart, startPayload!);
    }
  }
  ...
  Future<void> acceptQRScanConfirmation() async {
    if (_method is _KeyVerificationMethodQRReciprocate &&
      ...
      await (_method as _KeyVerificationMethodQRReciprocate).acceptQRScanConfirmation();
    }
  }
  ...
  List<KeyVerificationEmoji> get sasEmojis {
    if (_method is _KeyVerificationMethodSas) {
      final numbers = _bytesToInt((_method as _KeyVerificationMethodSas).makeSas(6), 6);
  ...
  Future<void> maybeRequestSSSSSecrets([int i = 0]) async {
    ...
    if ((!encryption.crossSigning.enabled ||
            (encryption.crossSigning.enabled && (await encryption.crossSigning.isCached()))) &&
        (!encryption.keyManager.enabled ||
            (encryption.keyManager.enabled && (await encryption.keyManager.isCached())))) {
      // no need to request cache, we already have it
      return;
    }
    // ignore: unawaited_futures
    encryption.ssss.maybeRequestAll(_verifiedDevices.whereType<DeviceKeys>().toList());
  ...
  Future<void> verifyKeysQR(SignableKey key, {bool shower = true}) async {
    ...
    var askingSSSS = false;
    if (encryption.crossSigning.enabled && encryption.crossSigning.signable(keyList)) {
      // these keys can be signed! Let's do so
      if (await encryption.crossSigning.isCached()) {
        // we want to make sure the verification state is correct for the other party after this event is handled.
        // Otherwise the verification dialog might be stuck in an unverified but done state for a bit.
    ...
    if (room != null) {
      ...
      if (deviceId == '*') {
        if ({
          ...
          final deviceKeys = client.userDeviceKeys[userId]?.deviceKeys.values.where(
  ...
  Future<bool> verifyQrData(Uint8List qrDataRawBytes) async {
    ...
    final remoteQrMode = QRMode.values.singleWhere((mode) => mode.code == data[7]);
    ...
    final randomSharedSecret = encodeBase64Unpadded(data.sublist(10 + encodedTxnLen + 32 + 32));

    /// `request.randomSharedSecretForQRCode` is overwritten below to send with `sendStart`
    if ({QRMode.verifyOtherUser, QRMode.verifySelfUntrusted}.contains(remoteQrMode)) {
  ...
  Future<(String, String)?> getKeys(QRMode mode) async {
    ...
    if (mode == QRMode.verifyOtherUser && ownMasterKey != null && otherMasterKey != null) {
    ...
    } else if (mode == QRMode.verifySelfTrusted && ownMasterKey != null && otherDeviceKey != null) {
      if (ownMasterKey.verified) {
        return (ownMasterKey.ed25519Key!, otherDeviceKey.ed25519Key!);
      }
    } else if (mode == QRMode.verifySelfUntrusted && ownMasterKey != null && ownDeviceKey != null) {
  ...
  Future<QRCode?> generateQrCode() async {
    ...
    randomSharedSecretForQRCode = encodeBase64Unpadded(uc.secureRandomBytes(11));
...
class _KeyVerificationMethodQRReciprocate extends _KeyVerificationMethod {
  ...
  Future<void> acceptQRScanConfirmation() async {
    ...
    if (ourQRMode == QRMode.verifyOtherUser) {
      ...
      keyToVerify = client.userDeviceKeys[client.userID]?.deviceKeys[request.deviceId];
...
class MacMethod {
  static const hkdfHmacSha256V2 = 'hkdf-hmac-sha256.v2';
  static const hkdfHmacSha256V1 = 'hkdf-hmac-sha256';
}
...
const knownHashesAuthentificationCodes = [
  MacMethod.hkdfHmacSha256V2,
  MacMethod.hkdfHmacSha256V1,
];
...
class _KeyVerificationMethodSas extends _KeyVerificationMethod {
  ...
  List<String> get knownAuthentificationTypes {
    final types = <String>[];
    if (request.client.verificationMethods.contains(KeyVerificationMethod.emoji)) {
      types.add('emoji');
    }
    if (request.client.verificationMethods.contains(KeyVerificationMethod.numbers)) {
      types.add('decimal');
    }
    return types;
  }
  ...
  bool validateStart(Map<String, dynamic> payload) {
    ...
    if (possibleMessageAuthenticationCodes.contains(MacMethod.hkdfHmacSha256V2)) {
      possibleMessageAuthenticationCodes.remove(MacMethod.hkdfHmacSha256V1);
    }
  ...
  bool _handleAccept(Map<String, dynamic> payload) {
    if (!knownKeyAgreementProtocols.contains(payload['key_agreement_protocol'])) {
      return false;
    }
    ...
    hash = payload['hash'];
    if (!knownHashesAuthentificationCodes.contains(payload['message_authentication_code'])) {
      return false;
    }
  ...
  Future<void> _sendMac() async {
    ...
    mac[deviceKeyId] = _calculateMac(encryption.fingerprintKey!, baseInfo + deviceKeyId);
    ...
    if (masterKey != null && masterKey.verified) {
      ...
      mac[masterKeyId] = _calculateMac(masterKey.publicKey!, baseInfo + masterKeyId);
  ...
  String _calculateMac(String input, String info) {
    switch (messageAuthenticationCode) {
      case MacMethod.hkdfHmacSha256V2:
        return sas!.calculate_mac_fixed_base64(input, info);
      case MacMethod.hkdfHmacSha256V1:
        return sas!.calculate_mac(input, info);
      default:
        throw Exception('Unknown message authentification code $messageAuthenticationCode');
    }
  }
}
