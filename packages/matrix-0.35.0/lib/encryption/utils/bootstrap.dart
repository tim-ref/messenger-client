...
class Bootstrap {
  ...
  Map<String, Set<String>> analyzeSecrets() {
    ...
    for (final entry in client.accountData.entries) {
      ...
      final encryptedContent = event.content.tryGetMap<String, Object?>('encrypted');
      ...
      for (final keyEntry in encryptedContent.entries) {
        ...
        if (value['iv'] is! String || value['ciphertext'] is! String || value['mac'] is! String) {
          invalidKeys.add(key);
          continue;
        }
    ...
    try {
      ...
      if (oldSsssKeys != null) {
        ...
        Set<String> removeKey(String key) {
          final s = secrets.entries.where((e) => e.value.contains(key)).map((e) => e.key).toSet();
          secrets.removeWhere((k, v) => v.contains(key));
          return s;
        }
    ...
    try {
      ...
      if (setupMasterKey) {
        ...
        try {
          ...
          secretsToStore[EventTypes.CrossSigningMasterKey] = base64.encode(masterSigningKey);
        } finally {
          master.free();
        }
      } else {
      ...
      if (setupSelfSigningKey) {
        ...
        try {
          ...
          secretsToStore[EventTypes.CrossSigningSelfSigning] = base64.encode(selfSigningPriv);
        } finally {
          selfSigning.free();
        }
      }
      if (setupUserSigningKey) {
        ...
        try {
          ...
          secretsToStore[EventTypes.CrossSigningUserSigning] = base64.encode(userSigningPriv);
        } finally {
          userSigning.free();
        }
      }
      ...
      if (masterKey != null) {
        if (client.userDeviceKeys[client.userID]?.masterKey?.ed25519Key != masterKey.publicKey) {
          throw BootstrapBadStateException(
            'ERROR: New master key does not match up!',
          );
        }
        Logs().v('Set own master key to verified...');
        await client.userDeviceKeys[client.userID]!.masterKey!.setVerified(true, false);
