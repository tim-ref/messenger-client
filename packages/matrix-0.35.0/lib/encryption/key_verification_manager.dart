...
class KeyVerificationManager {
  ...
  Future<void> handleToDeviceEvent(ToDeviceEvent event) async {
    ...
    if (request != null) {
      ...
      final newKeyRequest = KeyVerification(encryption: encryption, userId: event.sender);
  ...
  Future<void> handleEventUpdate(EventUpdate update) async {
    ...
    final transactionId = KeyVerification.getTransactionId(event['content']) ?? event['event_id'];
    ...
    if (req != null) {
      ...
      if (!{EventTypes.KeyVerificationRequest, EventTypes.KeyVerificationStart}.contains(type)) {
        return; // we can only start on these
      }
      final room = client.getRoomById(update.roomID) ?? Room(id: update.roomID, client: client);
