...
class CrossSigning {
  ...
  CrossSigning(this.encryption) {
    encryption.ssss.setValidator(EventTypes.CrossSigningSelfSigning, (String secret) async {
    ...
    encryption.ssss.setValidator(EventTypes.CrossSigningUserSigning, (String secret) async {
      final keyObj = olm.PkSigning();
      try {
        return keyObj.init_with_seed(base64decodeUnpadded(secret)) ==
  ...
  Future<bool> isCached() async {
    ...
    return (await encryption.ssss.getCached(EventTypes.CrossSigningSelfSigning)) != null &&
        (await encryption.ssss.getCached(EventTypes.CrossSigningUserSigning)) != null;
  }
  ...
  Future<void> selfSign({
    ...
    final userDeviceKeys = client.userDeviceKeys[client.userID]?.deviceKeys[client.deviceID];
  ...
  bool signable(List<SignableKey> keys) => keys.any(
        (key) =>
            ...
            key is DeviceKeys && key.userId == client.userID && key.identifier != client.deviceID,
      );
  ...
  Future<void> sign(List<SignableKey> keys) async {
    ...
    void addSignature(
      ...
      ((signedKey.signatures ??= <String, Map<String, String>>{})[signedWith.userId] ??=
    ...
    if (signedKeys.isNotEmpty) {
      ...
      for (final key in signedKeys) {
        if (key.identifier == null || key.signatures == null || key.signatures?.isEmpty != false) {
          continue;
        }
        ...
        if (payload[key.userId]?[key.identifier]?['signatures'] != null) {
          ...
          payload[key.userId]![key.identifier]!['signatures'].addAll(key.signatures);
        } else {
