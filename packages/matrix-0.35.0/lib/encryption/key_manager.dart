...
class KeyManager {
  ...
  Future<void> setInboundGroupSession(
    ...
    final oldFirstIndex = oldSession?.inboundGroupSession?.first_known_index() ?? 0;
    ...
    final roomInboundGroupSessions = _inboundGroupSessions[roomId] ??= <String, SessionKey>{};
    ...
    final storeFuture = client.database
      ...
      if (uploaded) {
        await client.database?.markInboundGroupSessionAsUploaded(roomId, sessionId);
      }
    });
            ...
            await client.handleSync(
              SyncUpdate(
                ...
                rooms: switch (room.membership) {
                  Membership.join => RoomsUpdate(join: {room.id: JoinedRoomUpdate()}),
                  Membership.ban ||
                  Membership.leave =>
                    RoomsUpdate(leave: {room.id: LeftRoomUpdate()}),
                  Membership.invite => RoomsUpdate(invite: {room.id: InvitedRoomUpdate()}),
                  Membership.knock => RoomsUpdate(knock: {room.id: KnockRoomUpdate()}),
                },
              ),
            );
          });
  ...
  void maybeAutoRequest(
    ...
    if (room != null && !_requestedSessionIds.contains(requestIdent) && !client.isUnknownSession) {
      // do e2ee recovery
  ...
  Future<SessionKey?> loadInboundGroupSession(
    ...
    final session = await client.database?.getInboundGroupSession(roomId, sessionId);
    if (session == null) return null;
    final userID = client.userID;
    if (userID == null) return null;
    final dbSess = SessionKey.fromDb(session, userID);
    final roomInboundGroupSessions = _inboundGroupSessions[roomId] ??= <String, SessionKey>{};
    if (!dbSess.isValid || dbSess.sessionId.isEmpty || dbSess.sessionId != sessionId) {
      return null;
    }
  ...
  Future<bool> clearOrUseOutboundGroupSession(
    ...
    if (!wipe) {
      ...
      final encryptionContent = room.getState(EventTypes.Encryption)?.parsedRoomEncryptionContent;
      final maxMessages = encryptionContent?.rotationPeriodMsgs ?? 100;
      final maxAge = encryptionContent?.rotationPeriodMs ?? 604800000; // default of one week
      if ((sess.sentMessages ?? maxMessages) >= maxMessages ||
          sess.creationTime.add(Duration(milliseconds: maxAge)).isBefore(DateTime.now())) {
        wipe = true;
      }
    }
    ...
    if (!wipe) {
      ...
      if (oldUserIds.difference(newUserIds).isNotEmpty) {
        ...
        if (newUsers.isNotEmpty) {
          ...
          devicesToReceive.addAll(newDeviceKeys.where((d) => newUsers.contains(d.userId)));
        }
        ...
        for (final userId in oldUserIds) {
          final oldBlockedDevices = sess.devices.containsKey(userId)
              ? Set.from(
                  sess.devices[userId]!.entries.where((e) => e.value).map((e) => e.key),
                )
              : <String>{};
          final newBlockedDevices = newDeviceKeyIds.containsKey(userId)
              ? Set.from(
                  newDeviceKeyIds[userId]!.entries.where((e) => e.value).map((e) => e.key),
                )
              : <String>{};
        ...
        try {
          ...
          if (devicesToReceive.isNotEmpty) {
            ...
            for (final device in devicesToReceive) {
              ...
              if (!inboundSess.allowedAtIndex[device.userId]!.containsKey(device.curve25519Key) ||
  ...
  final Map<String, Future<OutboundGroupSession>> _pendingNewOutboundGroupSessions = {};
  ...
  Future<OutboundGroupSession> createOutboundGroupSession(String roomId) async {
    ...
    final newSess = _pendingNewOutboundGroupSessions[roomId] = _createOutboundGroupSession(roomId);

    try {
      await newSess;
    } finally {
      _pendingNewOutboundGroupSessions.removeWhere((_, value) => value == newSess);
    }
  ...
  Future<OutboundGroupSession> _createOutboundGroupSession(
    ...
    for (final device in deviceKeys) {
      ...
      allowedAtIndex[device.userId]![device.curve25519Key!] = outboundGroupSession.message_index();
    }
  ...
  Future<GetRoomKeysVersionCurrentResponse> getRoomKeysBackupInfo([
    ...
    if (_roomKeysVersionCache != null &&
        ...
        useCache &&
        DateTime.now().subtract(Duration(minutes: 5)).isBefore(_roomKeysVersionCacheDate!)) {
  ...
  Future<void> loadFromResponse(RoomKeys keys) async {
    ...
    final privateKey = base64decodeUnpadded((await encryption.ssss.getCached(megolmKey))!);
        ...
        for (final sessionEntry in roomEntry.value.sessions.entries) {
          ...
          if (decrypted != null && senderKey != null) {
            ...
            await setInboundGroupSession(
              ...
              forwarded: true,
              senderClaimedKeys:
                  decrypted.tryGetMap<String, String>('sender_claimed_keys') ?? <String, String>{},
              uploaded: true,
            );
          }
        }
  ...
  Future<void> loadSingleKey(String roomId, String sessionId) async {
    final info = await getRoomKeysBackupInfo();
    final ret = await client.getRoomKeyBySessionId(roomId, sessionId, info.version);
    final keys = RoomKeys.fromJson({
      'rooms': {
        roomId: {
          'sessions': {
  ...
  Future<void> request(
    ...
    if (tryOnlineBackup && await isCached()) {
      ...
      if (!hadPreviously && getInboundGroupSession(room.id, sessionId) != null) {
      ...
      try {
        ...
        try {
          ...
          for (final dbSession in dbSessions) {
            final device = client.getUserDeviceKeysByCurve25519Key(dbSession.senderKey);
            args.dbSessions.add(
          ...
          final roomKeys = await client.nativeImplementations.generateUploadKeys(args);
  ...
  Future<void> handleToDeviceEvent(ToDeviceEvent event) async {
    if (event.type == EventTypes.RoomKeyRequest) {
      ...
      if (event.content['action'] == 'request') {
        ...
        final device =
            client.userDeviceKeys[event.sender]?.deviceKeys[event.content['requesting_device_id']];
        if (device == null) {
          Logs().w('[KeyManager] Device not found, doing nothing');
          return; // device not found
        }
        if (device.userId == client.userID && device.deviceId == client.deviceID) {
        ...
        final roomKeyRequest = RoomKeyRequest.fromToDeviceEvent(event, this, request);
        if (device.userId == client.userID && device.verified && !device.blocked) {
          ...
          final index = session.allowedAtIndex[device.userId]![device.curve25519Key]!;
          Logs().i(
            '[KeyManager] Valid foreign request, forwarding key at index $index...',
          );
          await roomKeyRequest.forwardKey(index);
        } else {
          Logs().i('[KeyManager] Asking client, if the key should be forwarded');
          client.onRoomKeyRequest.add(roomKeyRequest); // let the client handle this
        }
      ...
      final request = outgoingShareRequests.values.firstWhereOrNull(
        (r) => r.room.id == event.content['room_id'] && r.sessionId == event.content['session_id'],
      );
      ...
      final device = request.devices.firstWhereOrNull(
        (d) => d.userId == event.sender && d.curve25519Key == encryptedContent['sender_key'],
      );
      if (device == null) {
...
class RoomKeyRequest extends ToDeviceEvent {
  ...
  Future<void> forwardKey([int? index]) async {
    ...
    message['sender_claimed_ed25519_key'] = session.senderClaimedKeys['ed25519'] ??
...
RoomKeys generateUploadKeysImplementation(GenerateUploadKeysArgs args) {
  ...
  try {
    ...
    for (final dbSession in args.dbSessions) {
      ...
      final roomKeyBackup = roomKeys.rooms[sess.roomId] ??= RoomKeyBackup(sessions: {});
      ...
      final payload = <String, dynamic>{
        ...
        'session_key':
            sess.inboundGroupSession!.export_session(sess.inboundGroupSession!.first_known_index()),
      };
      // encrypt the content
...
class DbInboundGroupSessionBundle {
  ...
  factory DbInboundGroupSessionBundle.fromJson(Map<dynamic, dynamic> json) =>
      DbInboundGroupSessionBundle(
        dbSession: StoredInboundGroupSession.fromJson(Map.from(json['dbSession'])),
...
class GenerateUploadKeysArgs {
  ...
  factory GenerateUploadKeysArgs.fromJson(Map<dynamic, dynamic> json) => GenerateUploadKeysArgs(
        pubkey: json['pubkey'],
