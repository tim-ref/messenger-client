...
class Encryption {
  ...
  DatabaseApi? get olmDatabase => ourDeviceId == client.deviceID ? client.database : null;
  ...
  Future<void> handleToDeviceEvent(ToDeviceEvent event) async {
    ...
    if ([EventTypes.RoomKeyRequest, EventTypes.ForwardedRoomKey].contains(event.type)) {
  ...
  Future<void> handleEventUpdate(EventUpdate update) async {
    if (update.type == EventUpdateType.ephemeral || update.type == EventUpdateType.history) {
      return;
    }
    if (update.content['type'].startsWith('m.key.verification.') ||
        (update.content['type'] == EventTypes.Message &&
            ...
            update.content['content']['msgtype'].startsWith('m.key.verification.'))) {
  ...
  Event decryptRoomEventSync(String roomId, Event event) {
    ...
    if (event.type != EventTypes.Encrypted || content.ciphertextMegolm == null) {
      return event;
    }
    ...
    try {
      ...
      final inboundGroupSession = keyManager.getInboundGroupSession(roomId, sessionId);
      ...
      final decryptResult =
          inboundGroupSession!.inboundGroupSession!.decrypt(content.ciphertextMegolm!);
      ...
      final messageIndexValue = '${event.eventId}|${event.originServerTs.millisecondsSinceEpoch}';
      final haveIndex = inboundGroupSession.indexes.containsKey(messageIndexKey);
      if (haveIndex && inboundGroupSession.indexes[messageIndexKey] != messageIndexValue) {
      ...
      if (exception.toString() != DecryptException.unknownSession &&
          (keyManager.getOutboundGroupSession(roomId)?.outboundGroupSession?.session_id() ?? '') ==
              content.sessionId) {
        runInRoot(
          () async => keyManager.clearOrUseOutboundGroupSession(roomId, wipe: true),
        );
      }
    ...
    if (event.content['m.relates_to'] != null) {
      decryptedPayload['content']['m.relates_to'] = event.content['m.relates_to'];
    }
    return Event(
  ...
  Future<Map<String, dynamic>> encryptGroupMessagePayload(
    ...
    final encryptedPayload = <String, dynamic>{
      ...
      'ciphertext': sess!.outboundGroupSession!.encrypt(json.encode(payloadContent)),
...
class DecryptException implements Exception {
  ...
  @override
  String toString() => cause + (libolmMessage != null ? ': $libolmMessage' : '');
  ...
  static const String unknownSession = 'The sender has not sent us the session key.';
  static const String channelCorrupted = 'The secure channel with the sender was corrupted.';
  static const String unableToDecryptWithAnyOlmSession =
      'Unable to decrypt with any existing OLM session';
  static const String senderDoesntMatch = "Message was decrypted but sender doesn't match";
  static const String recipientDoesntMatch = "Message was decrypted but recipient doesn't match";
  static const String ownFingerprintDoesntMatch =
      "Message was decrypted but own fingerprint Key doesn't match";
  static const String isntSentForThisDevice = "The message isn't sent for this device";
