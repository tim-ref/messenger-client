    ...
    if (memberships != null && memberships.isNotEmpty) {
      for (final mem in memberships) {
        if (isValidMemEvent(mem)) {
          final callMem = CallMembership.fromJson(mem, event.senderId, event.room.id);
...
class CallMembership {
  ...
  static CallMembership? fromJson(Map json, String userId, String roomId) {
    try {
      return CallMembership(
        ...
        scope: json['scope'],
        backend: (json['foci_active'] as List).map((e) => CallBackend.fromJson(e)).first,
        deviceId: json['device_id'],
        expiresTs: json['expires_ts'],
        membershipId: json['membershipID'] ?? 'someone_forgot_to_set_the_membershipID',
        feeds: json['feeds'],
      );
    } catch (e, s) {
  ...
  bool get isExpired =>
      expiresTs < DateTime.now().subtract(CallTimeouts.expireTsBumpDuration).millisecondsSinceEpoch;
}
