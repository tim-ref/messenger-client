...
extension FamedlyCallMemberEventsExtension on Room {
  ...
  Map<String, FamedlyCallMemberEvent> getFamedlyCallEvents() {
    ...
    final famedlyCallMemberStates = states.tryGetMap<String, Event>(EventTypes.GroupCallMember);
    ...
    for (final element in sortedEvents) {
      mappedEvents.addAll({element.senderId: FamedlyCallMemberEvent.fromJson(element)});
    }
    return mappedEvents;
  }
  ...
  Future<void> updateFamedlyCallMemberStateEvent(
    ...
    // ownership over them
    ownMemberships.removeWhere((element) => client.deviceID! == element.deviceId);
...
bool isValidMemEvent(Map<String, Object?> event) {
  if (event['call_id'] is String &&
    ...
    Logs().v('[VOIP] FamedlyCallMemberEvent ignoring unclean membership $event');
    return false;
  }
}
