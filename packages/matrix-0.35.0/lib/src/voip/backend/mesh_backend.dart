...
class MeshBackend extends CallBackend {
  ...
  final CachedStreamController<GroupCallSession> onGroupCallFeedsChanged = CachedStreamController();
  ...
  Future<MediaStream> _getUserMedia(
    ...
    try {
      return await groupCall.voip.delegate.mediaDevices.getUserMedia(mediaConstraints);
    } catch (e) {
  ...
  Future<MediaStream> _getDisplayMedia(GroupCallSession groupCall) async {
    ...
    try {
      return await groupCall.voip.delegate.mediaDevices.getDisplayMedia(mediaConstraints);
    } catch (e, s) {
  ...
  Future<void> _replaceCall(
    ...
    final existingCallIndex =
        _callSessions.indexWhere((element) => element.callId == existingCall.callId);
  ...
  Future<void> _onStreamsChanged(
    ...
    if (remoteStreamChanged) {
      ...
      } else if (currentUserMediaStream != null && remoteUsermediaStream != null) {
    ...
    final remoteScreenshareStreamChanged = remoteScreensharingStream != currentScreenshareStream;

    if (remoteScreenshareStreamChanged) {
      if (currentScreenshareStream == null && remoteScreensharingStream != null) {
        _addScreenshareStream(groupCall, remoteScreensharingStream);
      } else if (currentScreenshareStream != null && remoteScreensharingStream != null) {
  ...
  WrappedMediaStream? _getUserMediaStreamByParticipantId(String participantId) {
    final stream = _userMediaStreams.where((stream) => stream.participant.id == participantId);
  ...
  void _onActiveSpeakerLoop(GroupCallSession groupCall) async {
    ...
    final userMediaStreamsCopyList = List<WrappedMediaStream>.from(_userMediaStreams);
    for (final stream in userMediaStreamsCopyList) {
      ...
      statsReport.removeWhere((element) => !element.values.containsKey('audioLevel'));

      // https://www.w3.org/TR/webrtc-stats/#summary
      final otherPartyAudioLevel = statsReport
          .singleWhereOrNull(
            (element) => element.type == 'inbound-rtp' && element.values['kind'] == 'audio',
          )
          ?.values['audioLevel'];
      ...
      final ownAudioLevel = statsReport
          .singleWhereOrNull(
            (element) => element.type == 'media-source' && element.values['kind'] == 'audio',
          )
          ?.values['audioLevel'];
  ...
  WrappedMediaStream? _getScreenshareStreamByParticipantId(
    ...
    final stream = _screenshareStreams.where((stream) => stream.participant.id == participantId);
    if (stream.isNotEmpty) {
      return stream.first;
    }
    return null;
  ...
  void _addScreenshareStream(
    ...
    onStreamAdd.add(stream);
    groupCall.onGroupCallEvent.add(GroupCallStateChange.screenshareStreamsChanged);
  }
  ...
  Future<void> _removeScreenshareStream(
    ...
    final streamIndex =
        _screenshareStreams.indexWhere((stream) => stream.participant.id == stream.participant.id);
  ...
  Future<void> _onCallStateChanged(CallSession call, CallState state) async {
    ...
    if (call.localUserMediaStream != null && call.isMicrophoneMuted != audioMuted) {
      await call.setMicrophoneMuted(audioMuted);
    }

    final videoMuted = localUserMediaStream?.isVideoMuted() ?? true;

    if (call.localUserMediaStream != null && call.isLocalVideoMuted != videoMuted) {
  ...
  Future<void> _addUserMediaStream(
    ...
    onStreamAdd.add(stream);
    groupCall.onGroupCallEvent.add(GroupCallStateChange.userMediaStreamsChanged);
  }
  ...
  @override
  List<WrappedMediaStream> get userMediaStreams => List.unmodifiable(_userMediaStreams);

  @override
  List<WrappedMediaStream> get screenShareStreams => List.unmodifiable(_screenshareStreams);
  ...
  Future<void> _onIncomingCall(
    ...
    if (newCall.groupCallId == null || newCall.groupCallId != groupCall.groupCallId) {
      Logs().v(
  ...
  Future<void> setScreensharingEnabled(
    ...
    if (enabled) {
      try {
        ...
        groupCall.onGroupCallEvent.add(GroupCallStateChange.localScreenshareStateChanged);
        for (final call in _callSessions) {
