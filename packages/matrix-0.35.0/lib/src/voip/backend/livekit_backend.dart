...
class LiveKitBackend extends CallBackend {
  ...
  Future<void> _setEncryptionKey(
    ...
    final encryptionKeys = _encryptionKeysMap[participant] ?? <int, Uint8List>{};
  ...
  Future<void> _sendEncryptionKeysEvent(
    ...
    final sendKeysTo = sendTo ?? groupCall.participants.where((p) => !p.isLocal);
  ...
  Future<void> _sendToDeviceEvent(
    ...
    final txid = VoIP.customTxid ?? groupCall.client.generateUniqueTransactionId();
    final mustEncrypt = groupCall.room.encrypted && groupCall.client.encryptionEnabled;
    ...
    for (final participant in remoteParticipants) {
      ...
      if (mustEncrypt) {
        ...
        final deviceKey =
            groupCall.client.userDeviceKeys[participant.userId]?.deviceKeys[participant.deviceId];
        if (deviceKey != null) {
  ...
  Future<void> onCallEncryption(
    ...
    final p = CallParticipant(groupCall.voip, userId: userId, deviceId: deviceId);
  ...
  @override
  bool get isLocalVideoMuted => throw UnimplementedError('Use livekit sdk for this');

  @override
  bool get isMicrophoneMuted => throw UnimplementedError('Use livekit sdk for this');
