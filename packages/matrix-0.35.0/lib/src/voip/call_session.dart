...
class CallSession {
  ...
  User? get remoteUser =>
      remoteUserId != null ? room.unsafeGetUserFromMemoryOrFallback(remoteUserId!) : null;
  ...
  final CachedStreamController<CallSession> onCallStreamsChanged = CachedStreamController();

  final CachedStreamController<CallSession> onCallReplaced = CachedStreamController();

  final CachedStreamController<CallSession> onCallHangupNotifierForGroupCalls =
      CachedStreamController();

  final CachedStreamController<CallState> onCallStateChanged = CachedStreamController();

  final CachedStreamController<CallStateChange> onCallEventChanged = CachedStreamController();

  final CachedStreamController<WrappedMediaStream> onStreamAdd = CachedStreamController();

  final CachedStreamController<WrappedMediaStream> onStreamRemoved = CachedStreamController();
  ...
  Future<bool> hasVideoToSend() async {
    ...
    return localUserMediaVideoTrack != null &&
        transceivers.singleWhereOrNull(
              (transceiver) => transceiver.sender.track?.id == localUserMediaVideoTrack.id,
            ) !=
            null;
  }
  ...
  Future<void> initWithInvite(
    ...
    if (!isGroupCall) {
      ...
      if (prevCallId != null) {
        ...
        final prevCall = voip.calls[VoipId(roomId: room.id, callId: prevCallId)];
        if (prevCall != null) {
          if (prevCall._inviteOrAnswerSent) {
            ...
            if (callId.compareTo(prevCall.callId) > 0) {
              ...
              voip.currentCID = VoipId(roomId: room.id, callId: prevCall.callId);
            } else {
              Logs().d(
  ...
  Future<void> replacedBy(CallSession newCall) async {
    ...
    } else if (state == CallState.kCreateOffer || state == CallState.kInviteSent) {
  ...
  void _updateRemoteSDPStreamMetadata(SDPStreamMetadata metadata) {
    ...
    _remoteSDPStreamMetadata?.sdpStreamMetadatas.forEach((streamId, sdpStreamMetadata) {
      Logs().i(
    ...
    for (final wpstream in getRemoteStreams) {
      ...
      if (purpose != null) {
        wpstream.setAudioMuted(metadata.sdpStreamMetadatas[streamId]!.audio_muted);
        wpstream.setVideoMuted(metadata.sdpStreamMetadatas[streamId]!.video_muted);
  ...
  Future<void> deleteFeedByStream(MediaStream stream) async {
    final index = _streams.indexWhere((element) => element.stream!.id == stream.id);
    if (index == -1) {
  ...
  Future<void> insertVideoTrackToAudioOnlyStream() async {
    if (localUserMediaStream != null && localUserMediaStream!.stream != null) {
      ...
      if (stream != null) {
        ...
        for (final newTrack in streamTracks) {
          ...
          if (transceiver != null) {
            ...
            await transceiver.setDirection(
              await transceiver.getDirection() == TransceiverDirection.Inactive // upgrade, send now
        ...
        localUserMediaStream?.onStreamChanged.add(localUserMediaStream!.stream!);
      }
    }
  }
  ...
  Future<void> hangup({
    ...
    try {
      final res = await sendHangupCall(room, callId, localPartyId, 'userHangup');
  ...
  Future<void> terminate(
    ...
    if (!isGroupCall) {
      ...
      if (voip.currentCID != null && voip.currentCID != VoipId(roomId: room.id, callId: callId))
        return;
  ...
  Future<void> updateMuteStatus() async {
    final micShouldBeMuted =
        (localUserMediaStream != null && localUserMediaStream!.isAudioMuted()) || _remoteOnHold;
    final vidShouldBeMuted =
        (localUserMediaStream != null && localUserMediaStream!.isVideoMuted()) || _remoteOnHold;
  ...
  Future<MediaStream?> _getUserMedia(CallType type) async {
    final mediaConstraints = {
      ...
      'video': type == CallType.kVideo ? UserMediaConstraints.camMediaConstraints : false,
    };
    try {
  ...
  Future<void> tryRemoveStopedStreams() async {
    ...
    _streams.removeWhere((stream) => removedStreams.containsKey(stream.stream!.id));
  ...
  Future<String?> sendSelectCallAnswer(
    ...
    return await _sendContent(
      room,
      isGroupCall ? EventTypes.GroupCallMemberSelectAnswer : EventTypes.CallSelectAnswer,
      content,
      txid: txid,
    );
  }
  ...
  Future<String?> sendCallNegotiate(
    ...
    return await _sendContent(
      room,
      isGroupCall ? EventTypes.GroupCallMemberNegotiate : EventTypes.CallNegotiate,
      content,
      txid: txid,
    );
  }
  ...
  Future<String?> sendCallCandidates(
    ...
    return await _sendContent(
      room,
      isGroupCall ? EventTypes.GroupCallMemberCandidates : EventTypes.CallCandidates,
      content,
      txid: txid,
    );
  }
  ...
  Future<String?> sendCallReplaces(
    ...
    return await _sendContent(
      room,
      isGroupCall ? EventTypes.GroupCallMemberReplaces : EventTypes.CallReplaces,
      content,
      txid: txid,
    );
  }
  ...
  Future<String?> sendAssertedIdentity(
    ...
    return await _sendContent(
      room,
      isGroupCall ? EventTypes.GroupCallMemberAssertedIdentity : EventTypes.CallAssertedIdentity,
      content,
      txid: txid,
    );
  }
  ...
  Future<String?> _sendContent(
    ...
    if (isGroupCall && remoteDeviceId != null) {
      ...
      if (mustEncrypt) {
        ...
        if (client.userDeviceKeys[remoteUserId]?.deviceKeys[remoteDeviceId] != null) {
      ...
      final sendMessageContent = mustEncrypt
          ? await client.encryption!.encryptGroupMessagePayload(room.id, content, type: type)
          : content;
