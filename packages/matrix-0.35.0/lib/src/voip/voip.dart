...
class VoIP {
  ...
  final StreamController<GroupCallSession> onIncomingGroupCall = StreamController();
  ...
  Future<void> _handleCallEvent(BasicEventWithSender event) async {
    ...
    if (room == null) {
      ...
      return;
    } else if (!event.type.startsWith(EventTypes.GroupCallMemberEncryptionKeys)) {
      ...
      if (callId != null) {
        ...
        if (call == null &&
            !{EventTypes.CallInvite, EventTypes.GroupCallMemberInvite}.contains(event.type)) {
          Logs().w(
          ...
          if ((call.remotePartyId != null && call.remotePartyId == localPartyId)) {
            Logs().v(
  ...
  Future<void> onCallInvite(
    ...
    if (inviteeDeviceId != null && inviteeDeviceId != localParticipant?.deviceId) {
    ...
    if (!delegate.canHandleNewCall &&
        (confId == null || currentGroupCID != VoipId(roomId: room.id, callId: confId))) {
      Logs().v(
