...
class GroupCallSession {
  ...
  final CachedStreamController<GroupCallState> onGroupCallState = CachedStreamController();

  final CachedStreamController<GroupCallStateChange> onGroupCallEvent = CachedStreamController();

  final CachedStreamController<MatrixRTCCallEvent> matrixRTCEventStream = CachedStreamController();
  ...
  String get avatarName => _getUser().calcDisplayname(mxidLocalPartFallback: false);
  ...
  Future<void> sendMemberStateEvent() async {
    await room.updateFamedlyCallMemberStateEvent(
      CallMembership(
        ...
        expiresTs: DateTime.now().add(CallTimeouts.expireTsBumpDuration).millisecondsSinceEpoch,
    ...
    _resendMemberStateEventTimer = Timer.periodic(
      ...
      ((timer) async {
        ...
        if (state != GroupCallState.ended || state != GroupCallState.localCallFeedUninitialized) {
          await sendMemberStateEvent();
        } else {
          Logs().d(
  ...
  Future<void> onMemberStateChanged() async {
    ...
    final mems = room.getCallMembershipsFromRoom().values.expand((element) => element);
    ...
    final ignoredMems = mems.where((element) => !memsForCurrentGroupCall.contains(element));
    ...
    if (anyJoined.isNotEmpty || anyLeft.isNotEmpty) {
      if (anyJoined.isNotEmpty) {
        final nonLocalAnyJoined = Set<CallParticipant>.from(anyJoined)..remove(localParticipant);
        ...
        matrixRTCEventStream.add(ParticipantsJoinEvent(participants: anyJoined.toList()));
      }
      if (anyLeft.isNotEmpty) {
        final nonLocalAnyLeft = Set<CallParticipant>.from(anyLeft)..remove(localParticipant);
        ...
        matrixRTCEventStream.add(ParticipantsLeftEvent(participants: anyLeft.toList()));
      }
