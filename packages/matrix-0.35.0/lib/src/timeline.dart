...
class Timeline {
    ...
    try {
      ...
      if (eventsFromStore != null && eventsFromStore.isNotEmpty) {
        ...
        for (final event in events) {
          ...
          final dbUser = await room.client.database?.getUser(event.senderId, room);
  ...
  Future<int> getRoomEvents({
    ...
    final type = direction == Direction.b ? EventUpdateType.history : EventUpdateType.timeline;
    ...
    final newEvents = resp.chunk.map((e) => Event.fromMatrixEvent(e, room)).toList();
  ...
  Timeline({
    ...
    cancelSendEventSub = room.client.onCancelSendEvent.stream.listen(_cleanUpCancelledEvent);
  ...
  Future<void> setReadMarker({String? eventId, bool? public}) async {
    eventId ??= events.firstWhereOrNull((event) => event.status.isSynced)?.eventId;
  ...
  void _handleEventUpdate(EventUpdate eventUpdate, {bool update = true}) {
    try {
      ...
      if (eventUpdate.content['type'] == EventTypes.Redaction) {
        final index = _findEvent(event_id: eventUpdate.content.tryGet<String>('redacts'));
  ...
  Stream<(List<Event>, String?)> startSearch({
    ...
    searchFunc ??= (event) => event.body.toLowerCase().contains(searchTerm?.toLowerCase() ?? '');
    final found = <Event>[];
    ...
    if (sinceEventId != null) {
      prevBatch = (await room.client.getEventContext(room.id, sinceEventId)).end;
    }
    final encryption = room.client.encryption;
