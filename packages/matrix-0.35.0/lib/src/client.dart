...
extension TrailingSlash on Uri {
  Uri stripTrailingSlash() =>
      path.endsWith('/') ? replace(path: path.substring(0, path.length - 1)) : this;
}
...
class Client extends MatrixApi {
  ...
  Client(
    ...
    @Deprecated('Use [sendTimelineEventTimeout] instead.') int? sendMessageTimeoutSeconds,
    ...
    this.typingIndicatorTimeout = const Duration(seconds: 30),
        ...
        supportedLoginTypes = supportedLoginTypes ?? {AuthenticationTypes.password},
        verificationMethods = verificationMethods ?? <KeyVerificationMethod>{},
        nativeImplementations =
            compute != null ? NativeImplementationsIsolate(compute) : nativeImplementations,
        super(
  ...
  Future<void> refreshAccessToken() async {
    ...
    final tokenExpiresAt =
        expiresInMs == null ? null : DateTime.now().add(Duration(milliseconds: expiresInMs));
  ...
  LoginState get loginState => onLoginStateChanged.value ?? LoginState.loggedOut;
  ...
  bool get isUnknownSession => userDeviceKeys[userID]?.deviceKeys[deviceID]?.signed != true;
  ...
  void _updatePushrules() {
    final ruleset = TryGetPushRule.tryFromJson(
      _accountData[EventTypes.PushRules]?.content.tryGetMap<String, Object?>('global') ?? {},
    );
  ...
  String? getDirectChatFromUserId(String userId) {
    ...
    if (directChats is List<dynamic> && directChats.isNotEmpty) {
      ...
      if (potentialRooms.isNotEmpty) {
        return potentialRooms.fold<Room>(potentialRooms.first!, (Room prev, Room? r) {
          if (r == null) {
            return prev;
          }
    ...
    for (final room in rooms) {
      if (room.membership == Membership.invite &&
          ...
          room.getState(EventTypes.RoomMember, userID!)?.content['is_direct'] == true) {
        return room.id;
      }
    }
    return null;
  ...
  Future<
    ...
    final supportedVersions = overrideSupportedVersions ?? Client.supportedVersions;
    try {
      ...
      if (!versions.versions.any((version) => supportedVersions.contains(version))) {
  ...
  Future<LoginResponse> login(
    ...
    if (homeserver == null) {
      final domain = identifier is AuthenticationUserIdentifier ? identifier.user.domain : null;
      if (domain != null) {
  ...
  Future<String> startDirectChat(
    ...
    enableEncryption ??= encryptionEnabled && await userOwnsEncryptionKeys(mxid);
    if (enableEncryption) {
      initialState ??= [];
  ...
  Future<String> createGroupChat({
    ...
    enableEncryption ??= encryptionEnabled && preset != CreateRoomPreset.publicChat;
    if (enableEncryption) {
      initialState ??= [];
      if (!initialState.any((s) => s.type == EventTypes.Encryption)) {
        initialState.add(
  ...
  Future<CachedProfileInformation> getUserProfile(
    ...
    try {
      profile =
          await (_userProfileRequests[userId] ??= super.getUserProfile(userId).timeout(timeout));
    } catch (e) {
  ...
  final CachedStreamController<String> onUserProfileUpdate = CachedStreamController<String>();
  ...
  /// A_28342 - use includeLeave: true when loading archive.
  ...
  Future<void> _storeArchivedRoom(
    ...
    final archivedRoom = leftRoom ??
        Room(
          ...
          client: this,
          roomAccountData: roomUpdate.accountData?.asMap().map((k, v) => MapEntry(v.type, v)) ??
  ...
  final _versionsCache = AsyncCache<GetVersionsResponse>(const Duration(hours: 1));
  ...
  Future<Uri> uploadContent(
    ...
    final mxc = await super.uploadContent(file, filename: filename, contentType: contentType);
  ...
  PushRuleSet? get globalPushRules {
    final pushrules = _accountData['m.push_rules']?.content.tryGetMap<String, Object?>('global');
  ...
  PushRuleSet? get devicePushRules {
    final pushrules = _accountData['m.push_rules']?.content.tryGetMap<String, Object?>('device');
    return pushrules != null ? TryGetPushRule.tryFromJson(pushrules) : null;
  }
  ...
  final CachedStreamController<ToDeviceEvent> onToDeviceEvent = CachedStreamController();

  /// Tells you about to-device and room call specific events in sync
  final CachedStreamController<List<BasicEventWithSender>> onCallEvents = CachedStreamController();

  /// Called when the login state e.g. user gets logged out.
  final CachedStreamController<LoginState> onLoginStateChanged = CachedStreamController();
  ...
  final CachedStreamController<SdkError> onEncryptionError = CachedStreamController();
  ...
  final CachedStreamController<SyncStatusUpdate> onSyncStatus = CachedStreamController();
  ...
  final CachedStreamController<CachedPresence> onPresenceChanged = CachedStreamController();

  /// Callback will be called on account data updates.
  @Deprecated('Use `client.onSync` instead')
  final CachedStreamController<BasicEvent> onAccountData = CachedStreamController();

  /// Will be called when another device is requesting session keys for a room.
  final CachedStreamController<RoomKeyRequest> onRoomKeyRequest = CachedStreamController();

  /// Will be called when another device is requesting verification with this device.
  final CachedStreamController<KeyVerification> onKeyVerificationRequest = CachedStreamController();
  ...
  final CachedStreamController<UiaRequest> onUiaRequest = CachedStreamController();

  @Deprecated('This is not in use anywhere anymore')
  final CachedStreamController<Event> onGroupMember = CachedStreamController();

  final CachedStreamController<String> onCancelSendEvent = CachedStreamController();

  /// When a state in a room has been updated this will return the room ID
  /// and the state event.
  final CachedStreamController<({String roomId, StrippedStateEvent state})> onRoomState =
  ...
  Future<Event?> getEventByPushNotification(
    ...
    matrixEvent ??= await database?.getEventById(eventId, room).timeout(timeoutForServerRequests);

    try {
      matrixEvent ??= await getOneRoomEvent(roomId, eventId).timeout(timeoutForServerRequests);
      ...
      matrixEvent ??= notificationsResponse.notifications
          .firstWhereOrNull(
            (notification) =>
                notification.roomId == roomId && notification.event.eventId == eventId,
          )
          ?.event;
    }
    ...
    if (returnNullIfSeen) {
      ...
      final readMarkerEvent =
          await database?.getEventById(room.fullyRead, room).timeout(timeoutForServerRequests);
    ...
    try {
      await room.requestUser(matrixEvent.senderId).timeout(timeoutForServerRequests);
    } catch (e, s) {
    ...
    if (event.type == EventTypes.Encrypted && encryption != null) {
      ...
      if (decrypted.messageType == MessageTypes.BadEncrypted && prevBatch != null) {
        await oneShotSync();
  ...
  Future<void> init({
    ...
    if ((newToken != null || newUserID != null || newDeviceID != null || newDeviceName != null) &&
        (newToken == null || newUserID == null || newDeviceID == null || newDeviceName == null)) {
    ...
    try {
      ...
      if (account != null &&
        ...
        final tokenExpiresAtMs = int.tryParse(account.tryGet<String>('token_expires_at') ?? '');
        _accessTokenExpiresAt =
            tokenExpiresAtMs == null ? null : DateTime.fromMillisecondsSinceEpoch(tokenExpiresAtMs);
      ...
      if (database != null) {
        ...
        userDeviceKeysLoading =
            database.getUserDeviceKeys(this).then((keys) => _userDeviceKeys = keys);
  ...
  Future<void> clear() async {
    ...
    _id = accessToken =
        _syncFilterId = homeserver = _userID = _deviceID = _deviceName = _prevBatch = null;
    _rooms = [];
  ...
  Future<void> _sync({Duration? timeout}) {
    final currentSync = _currentSync ??= _innerSync(timeout: timeout).whenComplete(() {
      _currentSync = null;
  ...
  Future<void> _checkSyncFilter() async {
    ...
    if (syncFilterId == null && userID != null) {
      final syncFilterId = _syncFilterId = await defineFilter(userID, syncFilter);
  ...
  Future<void> _innerSync({Duration? timeout}) async {
    ...
    try {
      ...
      if (_currentSyncId != syncRequest.hashCode) {
        Logs().w('Current sync request ID has changed. Dropping this sync loop!');
        return;
      }
    ...
    for (final entry in roomsWithNewKeyToSessionId.entries) {
      ...
      if (room != null) {
        ...
        for (final event in _eventsPendingDecryption) {
          ...
          if (decryptedEvent.content.tryGet<String>('type') != EventTypes.Encrypted) {
        ...
        _eventsPendingDecryption.removeWhere(
          (e) => events.any(
            (decryptedEvent) => decryptedEvent.content['event_id'] == e.event.content['event_id'],
          ),
        );
      }
    }
  ...
  Future<void> _handleRooms(
    ...
    for (final entry in rooms.entries) {
      ...
      if (syncRoomUpdate is JoinedRoomUpdate && syncRoomUpdate.timeline?.limited == true) {
      ...
      final timelineUpdateType = direction != null
          ? (direction == Direction.b ? EventUpdateType.history : EventUpdateType.timeline)
          : EventUpdateType.timeline;
    ...
    for (final event in events) {
      ...
      if ((event.type == EventTypes.Encryption &&
          ...
          event.content.tryGet<String>('algorithm') !=
              room.getState(EventTypes.Encryption)?.content.tryGet<String>('algorithm'))) {
        continue;
      }

      var update = EventUpdate(roomID: room.id, type: type, content: event.toJson());
      ...
      if (prevBatch != null &&
          (type == EventUpdateType.timeline || type == EventUpdateType.decryptedTimelineQueue)) {
        if ((update.content.tryGet<String>('type')?.startsWith(CallConstants.callEventsRegxp) ??
            false)) {
  ...
  Future<Room> _updateRoomsByRoomUpdate(
    ...
    final room = found
        ...
        : (chatUpdate is JoinedRoomUpdate
            ? Room(
                ...
                highlightCount: chatUpdate.unreadNotifications?.highlightCount ?? 0,
                notificationCount: chatUpdate.unreadNotifications?.notificationCount ?? 0,
    ...
    // Update notification, highlight count and/or additional information
      ...
      rooms[roomIndex].membership = membership;
      rooms[roomIndex].notificationCount = chatUpdate.unreadNotifications?.notificationCount ?? 0;
      rooms[roomIndex].highlightCount = chatUpdate.unreadNotifications?.highlightCount ?? 0;
      ...
      if (summary != null) {
        final roomSummaryJson = rooms[roomIndex].summary.toJson()..addAll(summary.toJson());
        rooms[roomIndex].summary = RoomSummary.fromJson(roomSummaryJson);
      }
      // ignore: deprecated_member_use_from_same_package
      rooms[roomIndex].onUpdate.add(rooms[roomIndex].id);
      if ((chatUpdate.timeline?.limited ?? false) && requestHistoryOnLimitedTimeline) {
        Logs().v(
        ...
        if (event.relationshipType == RelationshipTypes.edit) {
          if (event.relationshipEventId == room.lastEvent?.eventId ||
              (room.lastEvent?.relationshipType == RelationshipTypes.edit &&
                  event.relationshipEventId == room.lastEvent?.relationshipEventId)) {
            room.lastEvent = event;
          }
          break;
        }
  ...
  DeviceKeys? getUserDeviceKeysByCurve25519Key(String senderKey) {
    for (final user in userDeviceKeys.values) {
      final device = user.deviceKeys.values.firstWhereOrNull((e) => e.curve25519Key == senderKey);
      if (device != null) {
        return device;
      }
    }
  ...
  Future<void> updateUserDeviceKeys({Set<String>? additionalUsers}) async {
    try {
      ...
      _userDeviceKeys.removeWhere((String userId, v) => !trackedUserIds.contains(userId));
      ...
      for (final userId in trackedUserIds) {
        final deviceKeysList = _userDeviceKeys[userId] ??= DeviceKeysList(userId, this);
        ...
        if (deviceKeysList.outdated != false &&
            (failure == null || DateTime.now().subtract(Duration(minutes: 5)).isAfter(failure))) {
          outdatedLists[userId] = [];
        }
      }
      ...
      if (outdatedLists.isNotEmpty) {
        ...
        if (deviceKeys != null) {
          for (final rawDeviceKeyListEntry in deviceKeys.entries) {
            ...
            final userKeys = _userDeviceKeys[userId] ??= DeviceKeysList(userId, this);
            ...
            for (final rawDeviceKeyEntry in rawDeviceKeyListEntry.value.entries) {
              ...
              if (entry.isValid &&
                ...
                if (!oldKeys.containsKey(deviceId)) {
                  final oldPublicKeys = await database.deviceIdSeen(userId, deviceId);
                  if (oldPublicKeys != null && oldPublicKeys != curve25519Key + ed25519Key) {
                    Logs().w(
        ...
        for (final crossSigningKeysEntry in crossSigningTypes.entries) {
          ...
          for (final crossSigningKeyListEntry in keys.entries) {
            ...
            final userKeys = _userDeviceKeys[userId] ??= DeviceKeysList(userId, this);
            final oldKeys = Map<String, CrossSigningKey>.from(userKeys.crossSigningKeys);
  ...
  Future<void> sendToDeviceEncryptedChunked(
    ...
    deviceKeys.removeWhere(
      (DeviceKeys k) => k.blocked || (k.userId == userID && k.deviceId == deviceID),
    );
    if (deviceKeys.isEmpty) return;
  ...
  Future<void> dispose({bool closeDatabase = true}) async {
    ...
    try {
      if (closeDatabase) {
        ...
        _database = null;
        await database?.close().catchError((e, s) => Logs().w('Failed to close database: ', e, s));
      }
  ...
  Future<void> _migrateFromLegacyDatabase({
    ...
    final tokenExpiresAtMs = int.tryParse(migrateClient.tryGet<String>('token_expires_at') ?? '');
...
class _EventPendingDecryption {
  ...
  bool get timedOut => addedAt.add(Duration(minutes: 5)).isBefore(DateTime.now());
