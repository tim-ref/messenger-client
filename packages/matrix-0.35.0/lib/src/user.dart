...
class User extends StrippedStateEvent {
  ...
  String? get displayName =>
      ...
      (membership == Membership.join ? null : prevContent?.tryGet<String>('displayname'));
  ...
  Uri? get avatarUrl {
    final uri = content.tryGet<String>('avatar_url') ??
        (membership == Membership.join ? null : prevContent?.tryGet<String>('avatar_url'));
  ...
  Future<CachedPresence> fetchCurrentPresence() => room.client.fetchCurrentPresence(id);
  ...
  bool get canChangeUserPowerLevel =>
      room.canChangePowerLevel && (powerLevel < room.ownPowerLevel || id == room.client.userID);

  @override
  bool operator ==(Object other) =>
      (other is User && other.id == id && other.room == room && other.membership == membership);
  ...
  String get mention {
    ...
    final displayName = this.displayName;
    if (displayName == null || displayName.isEmpty || {'[', ']', ':'}.any(displayName.contains)) {
      return id;
    }
    ...
    allUsersWithSameDisplayname.removeWhere(
      (user) =>
          user.id == id || (user.displayName?.isEmpty ?? true) || user.displayName != displayName,
    );
