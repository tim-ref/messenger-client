...
extension MxcUriExtension on Uri {
  ...
  Future<Uri> getDownloadUri(Client client) async {
    ...
    if (await client.authenticatedMediaSupported()) {
      uriPath = '_matrix/client/v1/media/download/$host${hasPort ? ':$port' : ''}$path';
    } else {
      uriPath = '_matrix/media/v3/download/$host${hasPort ? ':$port' : ''}$path';
    }
  ...
  Future<Uri> getThumbnailUri(
    ...
    String requestPath;
    if (await client.authenticatedMediaSupported()) {
      requestPath = '/_matrix/client/v1/media/thumbnail/$host${hasPort ? ':$port' : ''}$path';
    } else {
      requestPath = '/_matrix/media/v3/thumbnail/$host${hasPort ? ':$port' : ''}$path';
    }
