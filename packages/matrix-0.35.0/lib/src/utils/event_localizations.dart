...
abstract class EventLocalizations {
  ...
  static final Map<String, String Function(Event event, MatrixLocalizations i18n, String body)?>
      localizationsMap = {
    ...
    EventTypes.RoomCanonicalAlias: (event, i18n, body) => i18n.changedTheRoomInvitationLink(
          event.senderFromMemoryOrFallback.calcDisplayname(i18n: i18n),
        ),
    ...
    EventTypes.RoomJoinRules: (event, i18n, body) {
      final joinRules = JoinRules.values.firstWhereOrNull(
        (r) => r.toString().replaceAll('JoinRules.', '') == event.content['join_rule'],
      );
      if (joinRules == null) {
    ...
    EventTypes.RoomMember: (event, i18n, body) {
      ...
      final senderName = event.senderFromMemoryOrFallback.calcDisplayname(i18n: i18n);
      ...
      switch (event.roomMemberChangeType) {
        ...
        case RoomMemberChangeType.displayname:
          final newDisplayname = event.content.tryGet<String>('displayname') ?? '';
          final oldDisplayname = event.prevContent?.tryGet<String>('displayname') ?? '';
          return i18n.changedTheDisplaynameTo(oldDisplayname, newDisplayname);
        case RoomMemberChangeType.join:
          return userIsTarget ? i18n.youJoinedTheChat : i18n.joinedTheChat(targetName);
        ...
        case RoomMemberChangeType.kick:
          return userIsSender ? i18n.youKicked(targetName) : i18n.kicked(senderName, targetName);
    ...
    EventTypes.RoomTopic: (event, i18n, body) => i18n.changedTheChatDescriptionTo(
          event.senderFromMemoryOrFallback.calcDisplayname(i18n: i18n),
    ...
    EventTypes.GuestAccess: (event, i18n, body) {
      final guestAccess = GuestAccess.values.firstWhereOrNull(
        (r) => r.toString().replaceAll('GuestAccess.', '') == event.content['guest_access'],
      );
      if (guestAccess == null) {
    ...
    EventTypes.Encrypted: (event, i18n, body) => _localizedBodyNormalMessage(event, i18n, body),
    EventTypes.Message: (event, i18n, body) => _localizedBodyNormalMessage(event, i18n, body),
    EventTypes.Reaction: (event, i18n, body) => i18n.sentReaction(
          event.senderFromMemoryOrFallback.calcDisplayname(i18n: i18n),
          event.content.tryGetMap<String, Object?>('m.relates_to')?.tryGet<String>('key') ?? body,
        ),
  };
}
