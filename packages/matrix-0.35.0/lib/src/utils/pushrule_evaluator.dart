...
class _PatternCondition {
  ...
  _PatternCondition.fromEventMatch(PushCondition condition) {
    ...
    tempPat = RegExp.escape(tempPat).replaceAll('\\*', '.*').replaceAll('\\?', '.');
...
class PushruleEvaluator {
  ...
  PushruleEvaluator.fromRuleset(PushRuleSet ruleset) {
    ...
    for (final r in ruleset.sender ?? []) {
      if (r.enabled) {
        _sender_rules[r.ruleId] = EvaluatedPushRuleAction.fromActions(r.actions);
      }
    }
  }
  ...
  EvaluatedPushRuleAction match(Event event) {
    ...
    final displayName =
        event.room.unsafeGetUserFromMemoryOrFallback(event.room.client.userID!).displayName;
