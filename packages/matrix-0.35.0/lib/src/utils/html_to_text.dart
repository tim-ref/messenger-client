...
class HtmlToText {
  ...
  static String _parsePreContent(_ConvertOpts opts, Element node) {
    ...
    final match =
        RegExp(r'^<code([^>]*)>', multiLine: false, caseSensitive: false).firstMatch(text);
    if (match == null) {
    ...
    final language = RegExp(r'language-(\w+)', multiLine: false, caseSensitive: false)
  ...
  static String _parseUlContent(_ConvertOpts opts, Element node) {
    ...
    opts.listDepth--;
    final bulletPoint = _listBulletPoints[opts.listDepth % _listBulletPoints.length];
  ...
  static String _parseOlContent(_ConvertOpts opts, Element node) {
    ...
    final start = (startStr is String && RegExp(r'^[0-9]+$', multiLine: false).hasMatch(startStr))
        ? int.parse(startStr)
        : 1;
  ...
  static List<String> _listChildNodes(
    ...
    for (final child in node.nodes) {
      if (types != null &&
          types.isNotEmpty &&
          ((child is Text) ||
              ((child is Element) && !types.contains(child.localName!.toLowerCase())))) {
        continue;
      }
