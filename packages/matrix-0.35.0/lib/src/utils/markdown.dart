...
class SpoilerSyntax extends DelimiterSyntax {
  ...
  Iterable<Node>? close(
    ...
    final element = Element('span', searchingForReason ? children : newChildren);
...
class InlineLatexSyntax extends DelimiterSyntax {
  ...
  @override
  bool onMatch(InlineParser parser, Match match) {
    final element = Element('span', [Element.text('code', htmlEscape.convert(match[1]!))]);
...
class PillSyntax extends InlineSyntax {
  ...
  @override
  bool onMatch(InlineParser parser, Match match) {
    if (match.start > 0 && !RegExp(r'[\s.!?:;\(]').hasMatch(match.input[match.start - 1])) {
      parser.addNode(Text(match[0]!));
      return true;
    }
    final identifier = match[1]!;
    final element = Element.text('a', htmlEscape.convert(identifier));
    element.attributes['href'] = htmlAttrEscape.convert('https://matrix.to/#/$identifier');
    parser.addNode(element);
    return true;
  }
}
...
class MentionSyntax extends InlineSyntax {
  ...
  bool onMatch(InlineParser parser, Match match) {
    ...
    element.attributes['href'] = htmlAttrEscape.convert('https://matrix.to/#/$mention');
    parser.addNode(element);
    return true;
  }
}
