...
class NativeImplementationsWebWorker extends NativeImplementations {
  ...
  Future<void> _handleIncomingMessage(MessageEvent event) async {
    ...
    if (data['label'] == 'stacktrace') {
      ...
      final stackTrace = await onStackTrace.call(event.data['stacktrace'] as String);
  ...
  Future<MatrixImageFileResizedResponse?> shrinkImage(
    ...
    try {
      final result = await operation<Map<dynamic, dynamic>, Map<String, dynamic>>(
...
class WebWorkerData {
  ...
  factory WebWorkerData.fromJson(LinkedHashMap<dynamic, dynamic> data) => WebWorkerData(
        data['label'],
