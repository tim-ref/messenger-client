...
class DeviceKeysList {
  ...
  CrossSigningKey? getCrossSigningKey(String type) =>
      crossSigningKeys.values.firstWhereOrNull((key) => key.usage.contains(type));
  ...
  Future<KeyVerification> startVerification({
    ...
    if (userId != client.userID) {
      ...
      final request = KeyVerification(encryption: encryption, room: room, userId: userId);
      await request.start();
...
class SimpleSignableKey extends MatrixSignableKey {
  ...
  SimpleSignableKey.fromJson(Map<String, dynamic> super.json) : super.fromJson();
}
...
abstract class SignableKey extends MatrixSignableKey {
  ...
  bool get verified => identifier != null && (directVerified || crossVerified) && !(blocked);
  ...
  SignableKey.fromJson(Map<String, dynamic> super.json, this.client) : super.fromJson() {
    _verified = false;
    _blocked = false;
  }
  ...
  bool hasValidSignatureChain({
    ...
    if (visited_.contains(setKey) ||
        (onlyValidateUserIds_.isNotEmpty && !onlyValidateUserIds_.contains(userId))) {
    ...
    for (final signatureEntries in signatures!.entries) {
      ...
      for (final signatureEntry in signatureEntries.value.entries) {
        ...
        if (onlyValidateUserIds_.isNotEmpty && !onlyValidateUserIds_.contains(key.userId)) {
        ...
        final fullKeyIdBool =
            validSignatures?.tryGetMap<String, Object?>(otherUserId)?.tryGet<bool>(fullKeyId);
  ...
  bool operator ==(Object other) =>
      (other is SignableKey && other.userId == userId && other.identifier == identifier);
...
class CrossSigningKey extends SignableKey {
  ...
  bool get isValid =>
      userId.isNotEmpty && publicKey != null && keys.isNotEmpty && ed25519Key != null;
  ...
  Future<void> setVerified(bool newVerified, [bool sign = true]) async {
    ...
    await client.database?.setVerifiedUserCrossSigningKey(newVerified, userId, publicKey!);
  }
  ...
  Future<void> setBlocked(bool newBlocked) async {
    ...
    _blocked = newBlocked;
    await client.database?.setBlockedUserCrossSigningKey(newBlocked, userId, publicKey!);
  }
...
class DeviceKeys extends SignableKey {
  ...
  String? get deviceDisplayName => unsigned?.tryGet<String>('device_display_name');

  bool? _validSelfSignature;
  bool get selfSigned =>
      _validSelfSignature ??
      (_validSelfSignature = deviceId != null &&
          signatures?.tryGetMap<String, Object?>(userId)?.tryGet<String>('ed25519:$deviceId') !=
              null &&
  ...
  Future<void> setVerified(bool newVerified, [bool sign = true]) async {
    ...
    await super.setVerified(newVerified, sign);
    await client.database?.setVerifiedUserDeviceKey(newVerified, userId, deviceId!);
  }
  ...
  Future<void> setBlocked(bool newBlocked) async {
    ...
    _blocked = newBlocked;
    await client.database?.setBlockedUserDeviceKey(newBlocked, userId, deviceId!);
  }
  ...
  DeviceKeys.fromDb(Map<String, dynamic> dbEntry, Client client)
    ...
    _verified = dbEntry['verified'];
    _blocked = dbEntry['blocked'];
    lastActive = DateTime.fromMillisecondsSinceEpoch(dbEntry['last_active'] ?? 0);
  }
