...
class Event extends MatrixEvent {
  ...
  Map get infoMap => content.tryGetMap<String, Object?>('info') ?? <String, Object?>{};

  /// Gets the thumbnail info map of file events, or a blank map if nonepresent
  Map get thumbnailInfoMap =>
      infoMap['thumbnail_info'] is Map ? infoMap['thumbnail_info'] : <String, dynamic>{};
  ...
  String get thumbnailMimetype => thumbnailInfoMap['mimetype'] is String
      ...
      : (infoMap['thumbnail_file'] is Map && infoMap['thumbnail_file']['mimetype'] is String
  ...
  Uri? get thumbnailMxcUrl {
    final url = isThumbnailEncrypted ? infoMap['thumbnail_file']['url'] : infoMap['thumbnail_url'];
    return url is String ? Uri.tryParse(url) : null;
  }
  ...
  Future<Uri?> getAttachmentUri({
    ...
    final thisMxcUrl = useThumbnailMxcUrl ? infoMap['thumbnail_url'] : content['url'];
  ...
  Future<bool> isAttachmentInLocalStore({bool getThumbnail = false}) async {
    ...
    final storeable = thisInfoMap['size'] is int && thisInfoMap['size'] <= database.maxFileSize;
    ...
    if (canDownloadFileFromServer) {
      ...
      downloadCallback ??= (Uri url) async => (await httpClient.get(
            ...
            headers: {'authorization': 'Bearer ${room.client.accessToken}'},
              ...
              .bodyBytes;
      uint8list = await downloadCallback(await mxcUrl.getDownloadUri(room.client));
  ...
  String? get relationshipEventId {
    ...
    return relatesToMap?.tryGet<String>('event_id') ??
        relatesToMap?.tryGetMap<String, Object?>('m.in_reply_to')?.tryGet<String>('event_id');
  }
