...
const String messageSendingStatusKey = 'com.famedly.famedlysdk.message_sending_status';

const String fileSendingStatusKey = 'com.famedly.famedlysdk.file_sending_status';
...
class Room {
  ...
  Future<void> postLoad() async {
    ...
    final allStates = await client.database?.getUnimportantRoomEventStatesForRoom(
  ...
  String get fullyRead => roomAccountData['m.fully_read']?.content.tryGet<String>('event_id') ?? '';
  ...
  /// the session ID.
  final CachedStreamController<String> onSessionKeyReceived = CachedStreamController();
  ...
  String getLocalizedDisplayname([
    ...
    if (heroes.isNotEmpty) {
      final result = heroes
          ...
          .map(
            (hero) => unsafeGetUserFromMemoryOrFallback(hero).calcDisplayname(i18n: i18n),
          )
          .join(', ');
    ...
    if (membership == Membership.invite) {
      ...
      if (ownMember.senderId != ownMember.stateKey) {
        return i18n.invitedBy(
          unsafeGetUserFromMemoryOrFallback(ownMember.senderId).calcDisplayname(i18n: i18n),
        );
      }
    }
    if (membership == Membership.leave) {
      if (directChatMatrixID != null) {
        return i18n.wasDirectChatDisplayName(
          unsafeGetUserFromMemoryOrFallback(directChatMatrixID).calcDisplayname(i18n: i18n),
        );
      }
    }
  ...
  Uri? get avatar {
    ...
    final avatarUrl = getState(EventTypes.RoomAvatar)?.content.tryGet<String>('url');
    if (avatarUrl != null) {
  ...
  String? get directChatMatrixID {
    ...
    final mxId = client.directChats.entries.firstWhereOrNull((MapEntry<String, dynamic> e) {
  ...
  List<User> get typingUsers {
    ...
    return (typingMxid is List)
        ? typingMxid.cast<String>().map(unsafeGetUserFromMemoryOrFallback).toList()
        : [];
  }
  ...
  bool get isAbandonedDMRoom {
    ...
    final dmPartnerMembership = unsafeGetUserFromMemoryOrFallback(directChatMatrixID).membership;
  ...
  Map<String, Tag> get tags {
    ...
    if (tags is Map) {
      final parsedTags = tags.map((k, v) => MapEntry<String, Tag>(k, _tryTagFromJson(v)));
  ...
  bool get hasNewMessages {
    ...
    if (lastEvent == null || !client.roomPreviewLastEvents.contains(lastEvent.type)) return false;

    // Read marker is on the last event so no new messages.
    if (lastEvent.receipts.any((receipt) => receipt.user.senderId == client.userID!)) {
      return false;
    }
  ...
  LatestReceiptState get receiptState => LatestReceiptState.fromJson(
        roomAccountData[LatestReceiptState.eventType]?.content ?? <String, dynamic>{},
      );
  ...
  Future<String> setPinnedEvents(List<String> pinnedEventIds) => client.setRoomStateWithKey(
        id,
  ...
  String? getMention(String mention) =>
      getParticipants().firstWhereOrNull((u) => u.mentionFragments.contains(mention))?.id;
  ...
  Future<EncryptionHealthState> calcEncryptionHealthState() async {
    ...
    if (users.any(
      (u) => client.userDeviceKeys[u.id]!.verified != UserVerifiedStatus.verified,
    )) {
  ...
  Future<String?> _sendContent(
    ...
    final sendMessageContent = mustEncrypt
        ? await client.encryption!.encryptGroupMessagePayload(id, content, type: type)
        : content;

    return await client.sendMessage(
      id,
      sendMessageContent.containsKey('ciphertext') ? EventTypes.Encrypted : type,
      txid,
      sendMessageContent,
    );
  }
  ...
  Future<String?> sendEvent(
    ...
    if (inReplyTo != null) {
      var replyText = '<${inReplyTo.senderId}> ${_stripBodyFallback(inReplyTo.body)}';
      ...
      final repliedHtml = content.tryGet<String>('formatted_body') ??
          htmlEscape.convert(content.tryGet<String>('body') ?? '').replaceAll('\n', '<br>');
    ...
    while (res == null) {
      try {
        ...
        if (e is MatrixException &&
            e.retryAfterMs != null &&
            !DateTime.now().add(Duration(milliseconds: e.retryAfterMs!)).isAfter(timeoutDate)) {
          Logs().w(
  ...
  Future<void> join({bool leaveIfNotFound = true}) async {
    try {
      ...
      if (leaveIfNotFound &&
          [MatrixError.M_NOT_FOUND, MatrixError.M_UNKNOWN].contains(exception.error)) {
        await leave();
      }
      rethrow;
    }
  ...
  Future<void> leave() async {
    try {
      ...
      if ([MatrixError.M_NOT_FOUND, MatrixError.M_UNKNOWN].contains(exception.error)) {
        await _handleFakeSync(
          SyncUpdate(
            nextBatch: '',
            rooms: RoomsUpdate(
  ...
  Future<String> setPower(String userId, int power) async {
    final powerLevelMapCopy = getState(EventTypes.RoomPowerLevels)?.content.copy() ?? {};
                ...
                ? {
                    id: JoinedRoomUpdate(
                      ...
                      timeline: TimelineUpdate(
                        limited: false,
                        events:
                            direction == Direction.b ? resp.chunk : resp.chunk.reversed.toList(),
                        prevBatch: direction == Direction.b ? resp.end : resp.start,
                      ),
                    ),
                  }
                : null,
  ...
  bool get isFederated =>
      getState(EventTypes.RoomCreate)?.content.tryGet<bool>('m.federate') ?? true;
  ...
  Future<List<User>> requestParticipants([
    ...
    final users = matrixEvents?.map((e) => Event.fromMatrixEvent(e, this).asUser).toList() ?? [];
  ...
  bool get participantListComplete {
    ...
    final joinedCount = knownParticipants.where((u) => u.membership == Membership.join).length;
    final invitedCount = knownParticipants.where((u) => u.membership == Membership.invite).length;
  ...
  Future<User?> _requestUser(
    ...
    if (requestProfile) {
      if (foundUser
          case null || User(membership: Membership.ban || Membership.leave, displayName: null)) {
        try {
    ...
    final userFromCurrentState = getState(EventTypes.RoomMember, mxID)?.asUser(this);
    ...
    if (userFromCurrentState == null || userFromCurrentState.displayName != foundUser.displayName) {
      setState(foundUser);
  ...
  int getPowerLevelByUserId(String userId) {
    ...
    final fallbackPowerLevel = getState(EventTypes.RoomCreate)?.senderId == userId ? 100 : 0;

    return userSpecificPowerLevel ?? defaultUserPowerLevel ?? fallbackPowerLevel;
  }
  ...
  Map<String, int>? get powerLevels {
    final powerLevelState = getState(EventTypes.RoomPowerLevels)?.content['users'];
  ...
  Future<String> setAvatar(MatrixFile? file) async {
    final uploadResp =
        file == null ? null : await client.uploadContent(file.bytes, filename: file.name);
    return await client.setRoomStateWithKey(
      id,
  ...
  bool get canBan =>
      (getState(EventTypes.RoomPowerLevels)?.content.tryGet<int>('ban') ?? 50) <= ownPowerLevel;
  ...
  int powerForChangingStateEvent(String action) {
    ...
    return powerLevelMap.tryGetMap<String, Object?>('events')?.tryGet<int>(action) ??
  ...
  Future<void> enableGroupCalls() async {
    ...
    if (currentPowerLevelsMap != null) {
      ...
      final eventsMap =
          newPowerLevelMap.tryGetMap<String, Object?>('events') ?? <String, Object?>{};
      eventsMap.addAll({
  ...
  bool get canInvite =>
      (getState(EventTypes.RoomPowerLevels)?.content.tryGet<int>('invite') ?? 0) <= ownPowerLevel;

  /// The level required to kick a user.
  bool get canKick =>
      (getState(EventTypes.RoomPowerLevels)?.content.tryGet<int>('kick') ?? 50) <= ownPowerLevel;

  /// The level required to redact an event.
  bool get canRedact =>
      (getState(EventTypes.RoomPowerLevels)?.content.tryGet<int>('redact') ?? 50) <= ownPowerLevel;
  ...
  bool get canSendDefaultStates {
    ...
    return (getState(EventTypes.RoomPowerLevels)?.content.tryGet<int>('state_default') ?? 50) <=
        ownPowerLevel;
  }

  bool get canChangePowerLevel => canChangeStateEvent(EventTypes.RoomPowerLevels);
  ...
  bool canSendEvent(String eventType) {
    ...
    final pl = powerLevelsMap?.tryGetMap<String, Object?>('events')?.tryGet<int>(eventType) ??
  ...
  PushRuleState get pushRuleState {
    final globalPushRules = client.accountData['m.push_rules']?.content['global'];
  ...
  JoinRules? get joinRules {
    final joinRulesString = getState(EventTypes.RoomJoinRules)?.content.tryGet<String>('join_rule');
    return JoinRules.values.singleWhereOrNull((element) => element.text == joinRulesString);
  }
  ...
  GuestAccess get guestAccess {
    final guestAccessString =
        getState(EventTypes.GuestAccess)?.content.tryGet<String>('guest_access');
  ...
  HistoryVisibility? get historyVisibility {
    final historyVisibilityString =
        getState(EventTypes.HistoryVisibility)?.content.tryGet<String>('history_visibility');
  ...
  bool get canChangeHistoryVisibility => canChangeStateEvent(EventTypes.HistoryVisibility);
  ...
  /// `m.space`.
  bool get isSpace =>
      getState(EventTypes.RoomCreate)?.content.tryGet<String>('type') == RoomCreationTypes.mSpace;
  ...
  Future<void> removeSpaceChild(String roomId) =>
      !isSpace ? throw Exception('Room is not a space!') : setSpaceChild(roomId, via: const []);
