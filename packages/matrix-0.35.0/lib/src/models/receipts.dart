    ...
    for (final eventIdEntry in json.entries) {
      ...
      for (final receiptTypeEntry in contentForEventId.entries) {
        ...
        for (final userIdEntry in contentForReceiptType.entries) {
          ...
          if (userId is! String || !userId.isValidMatrixId || receiptContent is! Map) continue;
...
class LatestReceiptStateForTimeline {
  ...
  factory LatestReceiptStateForTimeline.empty() => LatestReceiptStateForTimeline(
        ownPrivate: null,
        ownPublic: null,
  ...
  factory LatestReceiptStateForTimeline.fromJson(Map<String, dynamic> json) {
    ...
    final Map<String, LatestReceiptStateData> byUser =
        others?.map((k, v) => MapEntry(k, LatestReceiptStateData.fromJson(v))) ?? {};

    return LatestReceiptStateForTimeline(
      ownPrivate: private != null ? LatestReceiptStateData.fromJson(private) : null,
      ownPublic: public != null ? LatestReceiptStateData.fromJson(public) : null,
      latestOwnReceipt: latest != null ? LatestReceiptStateData.fromJson(latest) : null,
      otherUsers: byUser,
    );
  }
...
class LatestReceiptState {
  ...
  factory LatestReceiptState.fromJson(Map<String, dynamic> json) {
    ...
    return LatestReceiptState(
      ...
      mainThread: main.isNotEmpty ? LatestReceiptStateForTimeline.fromJson(main) : null,
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (byThread.isNotEmpty) 'thread': byThread.map((k, v) => MapEntry(k, v.toJson())),
      };
      ...
      receiptsByType.forEach((receiptType, receiptsByUser) {
        receiptsByUser.forEach((user, receipt) {
          ...
          if (threadId == 'main') {
            ...
            timeline = (byThread[threadId] ??= LatestReceiptStateForTimeline.empty());
          } else {
            timeline = global;
          }

          final receiptData = LatestReceiptStateData(eventId, receipt.originServerTs);
          if (user == ownUserid) {
