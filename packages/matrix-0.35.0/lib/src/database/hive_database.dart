...
class FamedlySdkHiveDatabase extends DatabaseApi with ZoneTransactionMixin {
  ...
  String get _outboundGroupSessionsBoxName => '$name.box.outbound_group_session';

  String get _olmSessionsBoxName => '$name.box.olm_session';

  String get _userDeviceKeysBoxName => '$name.box.user_device_keys';

  String get _userDeviceKeysOutdatedBoxName => '$name.box.user_device_keys_outdated';
  ...
  Future<void> _actionOnAllBoxes(Future<void> Function(BoxBase box) action) => Future.wait([
        action(_clientBox),
  ...
  @override
  Future<Map<String, BasicEvent>> getAccountData() => runBenchmarked<Map<String, BasicEvent>>(
  ...
  Future<List<String>> getLastSentMessageUserDeviceKey(
    ...
    final raw = await _userDeviceKeysBox.get(MultiKey(userId, deviceId).toString());
        ...
        () async {
          ...
          for (final key in _roomsBox.keys) {
            ...
            for (final userId in membersToPostload) {
              final state = await _roomMembersBox.get(MultiKey(room.id, userId).toString());
              if (state == null) {
            ...
            for (final type in importantRoomStates) {
              final states = await _roomStateBox.get(MultiKey(room.id, type).toString()) as Map?;
  ...
  @override
  Future<List<QueuedToDeviceEvent>> getToDeviceEventQueue() async => await Future.wait(
      ...
      runBenchmarked<Map<String, DeviceKeysList>>(
        ...
        () async {
          ...
          for (final userId in deviceKeysOutdated) {
            ...
            final crossSigningKeysBoxKeys = _userCrossSigningKeysBox.keys.where((tuple) {
              final tupleKey = MultiKey.fromString(tuple);
              return tupleKey.parts.first == userId;
            });
            res[userId] = DeviceKeysList.fromDbJson(
              ...
              await Future.wait(
                crossSigningKeysBoxKeys.map(
                  (key) async => convertToJson(await _userCrossSigningKeysBox.get(key)),
                ),
              ),
              client,
            );
  ...
  Future<void> removeUserCrossSigningKey(
    ...
    await _userCrossSigningKeysBox.delete(MultiKey(userId, publicKey).toString());
    return;
  }
  ...
  Future<void> setBlockedUserCrossSigningKey(
    ...
    final raw = await _userCrossSigningKeysBox.get(MultiKey(userId, publicKey).toString());
    raw['blocked'] = blocked;
  ...
  Future<void> setVerifiedUserCrossSigningKey(
    ...
    final raw =
        (await _userCrossSigningKeysBox.get(MultiKey(userId, publicKey).toString()) as Map?) ?? {};
  ...
  Future<void> storeEventUpdate(EventUpdate eventUpdate, Client client) async {
    ...
    if ({
      ...
      final Map? prevEvent = await _eventsBox.get(MultiKey(eventUpdate.roomID, eventId).toString());
      ...
      final key = MultiKey(eventUpdate.roomID, status.isSent ? '' : 'SENDING').toString();
  ...
  @override
  Future<void> transaction(Future<void> Function() action) => zoneTransaction(action);
  ...
  Future<List<StoredInboundGroupSession>> getAllInboundGroupSessions() async {
    final rawSessions = await Future.wait(
      _inboundGroupSessionsBox.keys.map((key) => _inboundGroupSessionsBox.get(key)),
    );
    return rawSessions
  ...
  Future<String?> deviceIdSeen(userId, deviceId) async {
    final raw = await _seenDeviceIdsBox.get(MultiKey(userId, deviceId).toString());
    if (raw == null) return null;
