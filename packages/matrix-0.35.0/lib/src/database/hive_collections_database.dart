...
class HiveCollectionsDatabase extends DatabaseApi {
  ...
  Future<List<Event>> _getEventsByIds(List<String> eventIds, Room room) async {
    ...
    return rawEvents
        .map(
          (rawEvent) => rawEvent != null ? Event.fromJson(copyMap(rawEvent), room) : null,
        )
        .whereNotNull()
        .toList();
  }
      ...
      runBenchmarked<List<Room>>('Get room list from store', () async {
        ...
        for (final room in rooms.values) {
          ...
          if (statesList != null) {
            ...
            // Load members
            final membersDbKeys =
                membersToPostload.map((member) => TupleKey(room.id, member).toString()).toList();
  ...
  Future<Map<String, DeviceKeysList>> getUserDeviceKeys(Client client) =>
      runBenchmarked<Map<String, DeviceKeysList>>('Get all user device keys from store', () async {
        final deviceKeysOutdated = await _userDeviceKeysOutdatedBox.getAllKeys();
        ...
        final userCrossSigningKeysBoxKeys = await _userCrossSigningKeysBox.getAllKeys();
        for (final userId in deviceKeysOutdated) {
          ...
          final crossSigningKeysBoxKeys = userCrossSigningKeysBoxKeys.where((tuple) {
            final tupleKey = TupleKey.fromString(tuple);
            return tupleKey.parts.first == userId;
          });
  ...
  Future<void> setBlockedUserCrossSigningKey(
    ...
    final raw = await _userCrossSigningKeysBox.get(TupleKey(userId, publicKey).toString());
    raw!['blocked'] = blocked;
