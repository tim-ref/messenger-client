...
class MatrixSdkDatabase extends DatabaseApi with DatabaseFileStorage {
  ...
  static const String _nonPreloadRoomStateBoxName = 'box_non_preload_room_states';
  ...
  static const String _inboundGroupSessionsBoxName = 'box_inbound_group_session';

  static const String _inboundGroupSessionsUploadQueueBoxName =
      'box_inbound_group_sessions_upload_queue';

  static const String _outboundGroupSessionsBoxName = 'box_outbound_group_session';

  static const String _olmSessionsBoxName = 'box_olm_session';

  static const String _userDeviceKeysBoxName = 'box_user_device_keys';

  static const String _userDeviceKeysOutdatedBoxName = 'box_user_device_keys_outdated';
  ...
  MatrixSdkDatabase(
    ...
    this.fileStorageLocation =
        fileStorageLocation ?? (legacyPath is String ? Uri.tryParse(legacyPath) : null);
  ...
  Future<void> forgetRoom(String roomId) async {
    ...
    final nonPreloadRoomStateBoxKeys = await _nonPreloadRoomStateBox.getAllKeys();
  ...
  Future<Map<String, BasicEvent>> getAccountData() =>
      runBenchmarked<Map<String, BasicEvent>>('Get all account data from store', () async {
  ...
  Future<List<Event>> getEventList(
    ...
    int? limit,
      ...
      runBenchmarked<List<Event>>('Get event list', () async {
        ...
        final timelineEventIds = (await _timelineFragmentsBox.get(timelineKey) ?? []);
  ...
  @override
  Future<List<StoredInboundGroupSession>> getInboundGroupSessionsToUpload() async {
    final uploadQueue = await _inboundGroupSessionsUploadQueueBox.getAllValues();
    final sessionFutures =
        uploadQueue.entries.take(50).map((entry) => getInboundGroupSession(entry.value, entry.key));
  ...
  Future<List<String>> getLastSentMessageUserDeviceKey(
    ...
    final raw = await _userDeviceKeysBox.get(TupleKey(userId, deviceId).toString());
  ...
  Future<List<OlmSession>> getOlmSessions(
    ...
    return rawSessions.values.map((json) => OlmSession.fromJson(copyMap(json), userId)).toList();
  }

  @override
  Future<Map<String, Map>> getAllOlmSessions() => _olmSessionsBox.getAllValues();
  ...
  Future<Room?> getSingleRoom(
    ...
    if (loadImportantStates) {
      final dbKeys =
          client.importantStateEvents.map((state) => TupleKey(roomId, state).toString()).toList();
      ...
      runBenchmarked<List<Room>>('Get room list from store', () async {
        ...
        for (final entry in roomAccountDataRaws.entries) {
          ...
          if (rooms.containsKey(roomId)) {
            ...
            await _roomAccountDataBox.delete(TupleKey(roomId, basicRoomEvent.type).toString());
          }
        }
  ...
  Future<User?> getUser(String userId, Room room) async {
    final state = await _roomMembersBox.get(TupleKey(room.id, userId).toString());
  ...
  Future<Map<String, DeviceKeysList>> getUserDeviceKeys(Client client) =>
      runBenchmarked<Map<String, DeviceKeysList>>('Get all user device keys from store', () async {
        final deviceKeysOutdated = await _userDeviceKeysOutdatedBox.getAllValues();
        if (deviceKeysOutdated.isEmpty) {
          return {};
        }
        final res = <String, DeviceKeysList>{};
        final userDeviceKeys = await _userDeviceKeysBox.getAllValues();
        final userCrossSigningKeys = await _userCrossSigningKeysBox.getAllValues();
        for (final userId in deviceKeysOutdated.keys) {
          ...
          res[userId] = DeviceKeysList.fromDbJson(
            ...
            childEntries.where((c) => c != null).toList().cast<Map<String, dynamic>>(),
            crossSigningEntries.where((c) => c != null).toList().cast<Map<String, dynamic>>(),
            client,
          );
        }
        return res;
  ...
  Future<void> removeEvent(String eventId, String roomId) async {
    ...
    for (final key in keys) {
      final multiKey = TupleKey.fromString(key);
      if (multiKey.parts.first != roomId) continue;
      final eventIds = List<String>.from(await _timelineFragmentsBox.get(key) ?? []);
  ...
  Future<void> removeUserCrossSigningKey(
    ...
    await _userCrossSigningKeysBox.delete(TupleKey(userId, publicKey).toString());
    return;
  }
  ...
  Future<void> setBlockedUserCrossSigningKey(
    ...
    final raw = copyMap(
      await _userCrossSigningKeysBox.get(TupleKey(userId, publicKey).toString()) ?? {},
    );
    raw['blocked'] = blocked;
  ...
  Future<void> storeEventUpdate(EventUpdate eventUpdate, Client client) async {
    ...
    final tmpRoom =
        client.getRoomById(eventUpdate.roomID) ?? Room(id: eventUpdate.roomID, client: client);

    // In case of this is a redaction event
    if (eventUpdate.content['type'] == EventTypes.Redaction) {
      final eventId = eventUpdate.content.tryGet<String>('redacts');
      final event = eventId != null ? await getEventById(eventId, tmpRoom) : null;
      if (event != null) {
    ...
    if ({EventUpdateType.timeline, EventUpdateType.history}.contains(eventUpdate.type)) {
      final eventId = eventUpdate.content['event_id'];
      // Is this ID already in the store?
      final prevEvent = await _eventsBox.get(TupleKey(eventUpdate.roomID, eventId).toString());
      final prevStatus = prevEvent == null
          ? null
          : () {
              final json = copyMap(prevEvent);
              final statusInt = json.tryGet<int>('status') ??
                  json.tryGetMap<String, dynamic>('unsigned')?.tryGet<int>(messageSendingStatusKey);
      ...
      final key = TupleKey(eventUpdate.roomID, status.isSent ? '' : 'SENDING').toString();
    ...
    if (currentRawRoom == null) {
      await _roomsBox.put(
        ...
        roomUpdate is JoinedRoomUpdate
            ? Room(
                ...
                highlightCount: roomUpdate.unreadNotifications?.highlightCount?.toInt() ?? 0,
                notificationCount: roomUpdate.unreadNotifications?.notificationCount?.toInt() ?? 0,
      ...
      await _roomsBox.put(
        ...
        Room(
          ...
          membership: membership,
          highlightCount:
              roomUpdate.unreadNotifications?.highlightCount?.toInt() ?? currentRoom.highlightCount,
          notificationCount: roomUpdate.unreadNotifications?.notificationCount?.toInt() ??
              currentRoom.notificationCount,
          prev_batch: roomUpdate.timeline?.prevBatch ?? currentRoom.prev_batch,
          summary: RoomSummary.fromJson(
            currentRoom.summary.toJson()..addAll(roomUpdate.summary?.toJson() ?? {}),
          ),
          lastEvent: lastEvent,
        ).toJson(),
      );
  ...
  @override
  Future<void> transaction(Future<void> Function() action) => _collection.transaction(action);
  ...
  Future<String?> deviceIdSeen(userId, deviceId) async {
    final raw = await _seenDeviceIdsBox.get(TupleKey(userId, deviceId).toString());
    if (raw == null) return null;
    return raw;
  }
  ...
  Future<String> exportDump() async {
    final dataMap = {
      ...
      _inboundGroupSessionsBoxName: await _inboundGroupSessionsBox.getAllValues(),
      _inboundGroupSessionsUploadQueueBoxName:
          await _inboundGroupSessionsUploadQueueBox.getAllValues(),
      _outboundGroupSessionsBoxName: await _outboundGroupSessionsBox.getAllValues(),
      _olmSessionsBoxName: await _olmSessionsBox.getAllValues(),
      _userDeviceKeysBoxName: await _userDeviceKeysBox.getAllValues(),
      _userDeviceKeysOutdatedBoxName: await _userDeviceKeysOutdatedBox.getAllValues(),
      _userCrossSigningKeysBoxName: await _userCrossSigningKeysBox.getAllValues(),
  ...
  Future<DiscoveryInformation?> getWellKnown() async {
    final rawDiscoveryInformation = await _clientBox.get('discovery_information');
  ...
  Future<CachedProfileInformation?> getUserProfile(String userId) =>
      _userProfilesBox.get(userId).then(
            (json) => json == null ? null : CachedProfileInformation.fromJson(copyMap(json)),
          );
