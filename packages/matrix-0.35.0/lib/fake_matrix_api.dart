...
class FakeMatrixApi extends BaseClient {
  ...
  static Map<String, List<dynamic>> get calledEndpoints => currentApi!._calledEndpoints;
  ...
  FutureOr<Response> mockIntercept(Request request) async {
    ...
    if (request.url.path.contains('/_matrix')) {
      action = '${request.url.path.split('/_matrix').last}?${request.url.query}';
    }
    ...
    final dynamic data = method == 'GET' ? request.url.queryParameters : request.body;
    dynamic res = {};
    var statusCode = 200;
    ...
    if (request.url.origin == 'https://fakeserverpriortoauthmedia.notexisting' &&
      ...
      if (_client != null) {
        ...
        for (final keyType in {
          ...
          if (jsonBody[keyType] != null) {
            final key = sdk.CrossSigningKey.fromJson(jsonBody[keyType], _client!);
            _client!.userDeviceKeys[_client!.userID!]?.crossSigningKeys
                .removeWhere((k, v) => v.usage.contains(key.usage.first));
            _client!.userDeviceKeys[_client!.userID!]?.crossSigningKeys[key.publicKey!] = key;
          }
        }
...
//        },
            ...
            'events': [
              {
                'content': {
                  ...
                  'msgtype': 'm.text',
                  'format': 'org.matrix.custom.html',
                  'formatted_body': '<b>This is a second text example message</b>',
                },
                'type': 'm.room.message',
              ...
              },
              {
                'content': {
                  ...
                  'msgtype': 'm.text',
                  'format': 'org.matrix.custom.html',
                  'formatted_body': '<b>This is a first text example message</b>',
                },
                'type': 'm.room.message',
  ...
  Map<String, Map<String, dynamic>> api = {
    'GET': {
      ...
      '/client/v1/media/preview_url?url=https%3A%2F%2Fmatrix.org&ts=10': (var req) => {
            'og:title': 'Matrix Blog Post',
            'og:description': 'This is a really cool blog post from matrix.org',
            'og:image': 'mxc://example.com/ascERGshawAWawugaAcauga',
            'og:image:type': 'image/png',
      ...
      '/client/v3/user/%40alice%3Aexample.com/rooms/!localpart%3Aexample.com/tags': (var req) => {
            'tags': {
      ...
      '/client/v3/account/whoami': (var req) => {'user_id': 'alice@example.com'},
      ...
      '/client/v3/user/%40alice%3Aexample.com/account_data/test.account.data': (var req) =>
          {'foo': 'bar'},
      ...
      '/client/v3/presence/${Uri.encodeComponent('@alice:example.com')}/status': (var req) => {
      ...
      '/client/v3/publicRooms?limit=10&since=1234&server=example.com': (var req) => {
            'chunk': [
              {
      ...
      '/client/v3/rooms/!1234%3Aexample.com/event/unencrypted_event': (var req) => {
            'content': {
              'body': 'This is an example text message',
              'msgtype': 'm.text',
              'format': 'org.matrix.custom.html',
      ...
      '/client/v3/register/available?username=testuser': (var req) => {'available': true},
    ...
    'POST': {
      ...
      '/client/v3/account/3pid/delete': (var req) => {'id_server_unbind_result': 'success'},
      '/client/v3/account/3pid/unbind': (var req) => {'id_server_unbind_result': 'success'},
      ...
      '/client/v3/account/deactivate': (var req) => {'id_server_unbind_result': 'success'},
      ...
      '/client/v3/rooms/!localpart%3Aexample.com/receipt/m.read/%241234%3Aexample.com': (var req) =>
          {},
      ...
      '/client/v3/user/${Uri.encodeComponent('@test:fakeServer.notExisting')}/filter': (var req) =>
          {'filter_id': '1234'},
      ...
      '/client/v3/keys/claim': (var req) => {
            ...
            'one_time_keys': {
              ...
              if (decodeJson(req)['one_time_keys']['@test:fakeServer.notExisting'] != null)
      ...
      '/client/v3/join/!newroom%3Aexample.com': (var req) => {'room_id': '!newroom%3A:example.com'},
      '/client/v3/keys/upload': (var req) => {
            'one_time_key_counts': {
              'curve25519': 10,
              'signed_curve25519': tryCast<Map<String, Object?>>(decodeJson(req))
            ...
            'device_keys': {
              '@alice:example.com': {
                'JLAFKJWSCS': {
                  ...
                  'keys': {
                    'curve25519:JLAFKJWSCS': 'L+4+JCl8MD63dgo8z5Ta+9QAHXiANyOVSfgbHA5d3H8',
                    'ed25519:JLAFKJWSCS': 'rUFJftIWpFF/jqqz3bexGGYiG8UobKhzkeabqw1v0zM',
                  },
                  'signatures': {
                    '@alice:example.com': {
                      'ed25519:JLAFKJWSCS':
                ...
                'OTHERDEVICE': {
                  ...
                  'keys': {
                    'curve25519:OTHERDEVICE': 'wMIDhiQl5jEXQrTB03ePOSQfR8sA/KMrW0CIfFfXKEE',
                    'ed25519:OTHERDEVICE': '2Lyaj5NB7HPqKZMjZpA/pECXuQ+9wi8AGFdw33y3DuQ',
                  },
                  'signatures': {
                    '@alice:example.com': {
                      'ed25519:OTHERDEVICE':
              ...
              '@test:fakeServer.notExisting': {
                'GHTYAJCE': {
                  ...
                  'keys': {
                    'curve25519:GHTYAJCE': '7rvl3jORJkBiK4XX1e5TnGnqz068XfYJ0W++Ml63rgk',
                    'ed25519:GHTYAJCE': 'gjL//fyaFHADt9KBADGag8g7F8Up78B/K1zXeiEPLJo',
                  },
                  'signatures': {
                    '@test:fakeServer.notExisting': {
                      'ed25519:GHTYAJCE':
                ...
                'OTHERDEVICE': {
                  ...
                  'keys': {
                    'curve25519:OTHERDEVICE': 'R96BA0qE1+QAWLp7E1jyWSTJ1VXMLpEdiM2SZHlKMXM',
                    'ed25519:OTHERDEVICE': 'EQo9eYbSygIbOR+tVJziqAY1NI6Gga+JQOVIqJe4mr4',
                  },
                  'signatures': {
                    '@test:fakeServer.notExisting': {
                      'ed25519:OTHERDEVICE':
      ...
      '/client/v3/register?kind=user': (var req) => {'user_id': '@testuser:example.com'},
      '/client/v3/register?kind=guest': (var req) => {'user_id': '@testuser:example.com'},
      ...
      '/client/v3/user/@test:fakeServer.notExisting/openid/request_token': (var req) => {
            'access_token': 'SomeT0kenHere',
            'token_type': 'Bearer',
            'matrix_server_name': 'example.com',
            'expires_in': 3600,
    ...
    'PUT': {
      ...
      '/client/v3/pushrules/global/content/nocake/enabled': (var req) => {},
      '/client/v3/pushrules/global/content/nocake/actions': (var req) => {},
      '/client/v3/rooms/!localpart%3Aserver.abc/state/m.room.history_visibility': (var req) =>
          {'event_id': '1234'},
      '/client/v3/rooms/!localpart%3Aserver.abc/state/m.room.join_rules': (var req) =>
          {'event_id': '1234'},
      '/client/v3/rooms/!localpart%3Aserver.abc/state/m.room.guest_access': (var req) =>
          {'event_id': '1234'},
      '/client/v3/rooms/!calls%3Aexample.com/send/m.call.invite/1234': (var req) =>
          {'event_id': '1234'},
      '/client/v3/rooms/!calls%3Aexample.com/send/m.call.answer/1234': (var req) =>
          {'event_id': '1234'},
      '/client/v3/rooms/!calls%3Aexample.com/send/m.call.select_answer/1234': (var req) =>
          {'event_id': '1234'},
      '/client/v3/rooms/!calls%3Aexample.com/send/m.call.reject/1234': (var req) =>
          {'event_id': '1234'},
      '/client/v3/rooms/!calls%3Aexample.com/send/m.call.negotiate/1234': (var req) =>
          {'event_id': '1234'},
      '/client/v3/rooms/!calls%3Aexample.com/send/m.call.candidates/1234': (var req) =>
          {'event_id': '1234'},
      '/client/v3/rooms/!calls%3Aexample.com/send/m.call.hangup/1234': (var req) =>
          {'event_id': '1234'},
      '/client/v3/rooms/!calls%3Aexample.com/send/m.call.replaces/1234': (var req) =>
          {'event_id': '1234'},
      '/client/v3/rooms/!calls%3Aexample.com/send/m.call.asserted_identity/1234': (var req) =>
          {'event_id': '1234'},
      ...
      '/client/v3/pushrules/global/override/.m.rule.master/enabled': (var req) => {},
      '/client/v3/pushrules/global/content/nocake?before=1&after=2': (var req) => {},
      '/client/v3/devices/QBUAZIFURK': (var req) => {},
      '/client/v3/directory/room/%23testalias%3Aexample.com': (var reqI) => {},
      '/client/v3/rooms/!localpart%3Aserver.abc/send/m.room.message/testtxid': (var reqI) => {
            'event_id': '\$event${FakeMatrixApi.eventCounter++}',
          },
      '/client/v3/rooms/!localpart%3Aserver.abc/send/m.reaction/testtxid': (var reqI) => {
            'event_id': '\$event${FakeMatrixApi.eventCounter++}',
          },
      '/client/v3/rooms/!localpart%3Aexample.com/typing/%40alice%3Aexample.com': (var req) => {},
      '/client/v3/rooms/!1234%3Aexample.com/send/m.room.message/1234': (var reqI) => {
            'event_id': '\$event${FakeMatrixApi.eventCounter++}',
          },
      '/client/v3/rooms/!1234%3Aexample.com/send/m.room.message/newresend': (var reqI) => {
            'event_id': '\$event${FakeMatrixApi.eventCounter++}',
          },
      ...
      '/client/v3/user/%40alice%3Aexample.com/rooms/!localpart%3Aexample.com/tags/testtag':
          (var req) => {},
      '/client/v3/user/%40alice%3Aexample.com/account_data/test.account.data': (var req) => {},
      '/client/v3/user/%40test%3AfakeServer.notExisting/account_data/best%20animal': (var req) =>
          {},
      '/client/v3/user/%40alice%3Aexample.com/rooms/1234/account_data/test.account.data':
          (var req) => {},
      '/client/v3/user/%40test%3AfakeServer.notExisting/rooms/!localpart%3Aserver.abc/account_data/m.marked_unread':
          (var req) => {},
      '/client/v3/user/%40test%3AfakeServer.notExisting/account_data/m.direct': (var req) => {},
      '/client/v3/user/%40othertest%3AfakeServer.notExisting/account_data/m.direct': (var req) =>
          {},
      '/client/v3/profile/%40alice%3Aexample.com/displayname': (var reqI) => {},
      '/client/v3/profile/%40alice%3Aexample.com/avatar_url': (var reqI) => {},
      '/client/v3/profile/%40test%3AfakeServer.notExisting/avatar_url': (var reqI) => {},
      '/client/v3/rooms/!localpart%3Aserver.abc/state/m.room.encryption': (var reqI) =>
          {'event_id': 'YUwRidLecu:example.com'},
      '/client/v3/rooms/!localpart%3Aserver.abc/state/m.room.avatar': (var reqI) =>
          {'event_id': 'YUwRidLecu:example.com'},
      '/client/v3/rooms/!localpart%3Aserver.abc/send/m.room.message/1234': (var reqI) =>
          {'event_id': 'YUwRidLecu:example.com'},
      '/client/v3/rooms/!localpart%3Aserver.abc/redact/1234/1234': (var reqI) =>
          {'event_id': 'YUwRidLecu:example.com'},
      '/client/v3/rooms/!localpart%3Aserver.abc/state/m.room.name': (var reqI) => {
            'event_id': '42',
          },
      '/client/v3/rooms/!localpart%3Aserver.abc/state/m.room.topic': (var reqI) => {
            'event_id': '42',
          },
      '/client/v3/rooms/!localpart%3Aserver.abc/state/m.room.pinned_events': (var reqI) => {
            'event_id': '42',
          },
      '/client/v3/rooms/!localpart%3Aserver.abc/state/m.room.power_levels': (var reqI) => {
            'event_id': '42',
          },
      '/client/v3/rooms/!calls%3Aexample.com/state/m.room.power_levels': (var reqI) => {
            'event_id': '42',
          },
      '/client/v3/directory/list/room/!localpart%3Aexample.com': (var req) => {},
    ...
    'DELETE': {
      ...
      '/client/v3/directory/room/%23testalias%3Aexample.com': (var reqI) => {},
      '/client/v3/pushrules/global/content/nocake': (var req) => {},
      '/client/v3/pushrules/global/override/!localpart%3Aserver.abc': (var req) => {},
      '/client/v3/user/%40test%3AfakeServer.notExisting/rooms/!localpart%3Aserver.abc/tags/m.favourite':
          (var req) => {},
      '/client/v3/user/%40alice%3Aexample.com/rooms/!localpart%3Aexample.com/tags/testtag':
          (var req) => {},
