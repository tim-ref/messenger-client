...
extension DehydratedDeviceHandler on Client {
  ...
  Future<void> dehydratedDeviceSetup(OpenSSSS secureStorage) async {
    try {
      ...
      if (_oldDehydratedDeviceAlgorithms.contains(device.deviceData?.tryGet<String>('algorithm'))) {
        await _uploadNewDevice(secureStorage);
        return;
      }
      ...
      if (device.deviceData?.tryGet<String>('algorithm') != _dehydratedDeviceAlgorithm) return;

      // Verify that the device is cross-signed
      final dehydratedDeviceIdentity = userDeviceKeys[userID]!.deviceKeys[device.deviceId];
      if (dehydratedDeviceIdentity == null || !dehydratedDeviceIdentity.hasValidSignatureChain()) {
        Logs().w(
      ...
      final pickleDeviceKey = await secureStorage.getStored(_ssssSecretNameForDehydratedDevice);
      final pickledDevice = device.deviceData?.tryGet<String>('device');
      if (pickledDevice == null) {
        Logs().w('Dehydrated device ${device.deviceId} is invalid, replacing it');
        await _uploadNewDevice(secureStorage);
        return;
      }
      ...
      try {
        ...
        if (dehydratedDeviceIdentity.curve25519Key != encryption.identityKey ||
          ...
          Logs().w('Invalid dehydrated device ${device.deviceId}, replacing it');
        ...
        await this.encryption?.keyManager.uploadInboundGroupSessions(skipIfInProgress: false);
  ...
  Future<void> _uploadNewDevice(OpenSSSS secureStorage) async {
    ...
    try {
      ...
      try {
        pickleDeviceKey = await secureStorage.getStored(_ssssSecretNameForDehydratedDevice);
      } catch (_) {
