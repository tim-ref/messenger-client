...
void main() {
  ...
  group('Room', () {
    ...
    test('setPower', () async {
      final powerLevelMap = room.getState(EventTypes.RoomPowerLevels, '')!.content.copy();
    ...
    test('sendEvent', () async {
      FakeMatrixApi.calledEndpoints.clear();
      final dynamic resp = await room.sendTextEvent('Hello world', txid: 'testtxid');
      expect(resp?.startsWith('\$event'), true);
    ...
    test('send reply', () async {
      ...
      expect(content, {
        'body': '> <@alice:example.org> <b>Blah</b>\n> beep\n\nHello world\nfox',
        'msgtype': 'm.text',
        'format': 'org.matrix.custom.html',
        'formatted_body':
      ...
      FakeMatrixApi.calledEndpoints.clear();
      resp = await room.sendTextEvent('Fox', txid: 'testtxid', inReplyTo: event);
      expect(resp?.startsWith('\$event'), true);
      entry = FakeMatrixApi.calledEndpoints.entries
          .firstWhere((p) => p.key.contains('/send/m.room.message/'));
      content = json.decode(entry.value.first);
    ...
    test('send reaction', () async {
      FakeMatrixApi.calledEndpoints.clear();
      final dynamic resp = await room.sendReaction('\$otherEvent', '🦊', txid: 'testtxid');
      expect(resp?.startsWith('\$event'), true);
      final entry = FakeMatrixApi.calledEndpoints.entries
    ...
    test('send location', () async {
      ...
      final dynamic resp = await room.sendLocation(body, geoUri, txid: 'testtxid');
      expect(resp?.startsWith('\$event'), true);
    ...
    test('pushRuleState', () async {
      ...
      ((matrix.accountData['m.push_rules']?.content['global'] as Map<String, Object?>)['override']
              as List)
          .add(
        ((matrix.accountData['m.push_rules']?.content['global'] as Map<String, Object?>)['room']
            as List)[0],
      );
    ...
    test('cancelSend because EventTooLarge in postLoaded room', () async {
      ...
      final sentEventFromDB = await matrix.database?.getEventById('older_event', room);
      ...
      // check if persisted in db
      final lastEventFromDB = await matrix.database?.getEventById('event_too_large', room);
