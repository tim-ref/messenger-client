  ...
  group('client mem', tags: 'olm', () {
    ...
    test('barebones client login', () async {
      ...
      await client.login(
        ...
        token: 'abcd',
        identifier: AuthenticationUserIdentifier(user: '@test:fakeServer.notExisting'),
        deviceId: 'GHTYAJCE',
      );
    ...
    test('Login', () async {
      ...
      try {
        await matrix.checkHomeserver(Uri.parse('https://fakeserver.wrongaddress'));
      } catch (exception) {
      ...
      expect(
        matrix.rooms[2].receiptState.global.otherUsers['@alice:example.com']?.ts,
        1436451550453,
      );
      expect(
        matrix.rooms[2].receiptState.global.otherUsers['@alice:example.com']?.eventId,
        '\$7365636s6r6432:example.com',
      );

      final inviteRoom = matrix.rooms.singleWhere((room) => room.membership == Membership.invite);
      ...
      expect(
        matrix.userDeviceKeys['@alice:example.com']?.deviceKeys['JLAFKJWSCS']?.verified,
        false,
      );
    ...
    test('roomAccountData', () async {
      ...
      final roomFromList =
          (await matrix.database?.getRoomList(matrix))?.firstWhere((room) => room.id == roomId);
    ...
    test('getProfileFromUserId', () async {
      ...
      final cachedProfileFromDb = await matrix.database?.getUserProfile('@alice:example.com');
    ...
    test('joinAfterInviteMembership', () async {
      ...
      final room = client.getRoomById('!bWEUQDujMKwjxkCXYr:tim-alpha.staging.famedly.de')!;
      await room.postLoad();
              ...
              roomId: LeftRoomUpdate(
                state: [
                  MatrixEvent(
                    ...
                    stateKey: client.userID,
                    originServerTs: DateTime.now(),
                    eventId: '\$abcdefwsjaskdfabsjfhabfsjgbahsjfkgbasjffsajfgsfd',
                    content: {
                      'membership': 'leave',
                    },
                  ),
    ...
    test('sendToDeviceEncrypted', () async {
      ...
      await matrix.sendToDeviceEncrypted(
          matrix.userDeviceKeys['@alice:example.com']!.deviceKeys.values.toList(), 'm.message', {
        'msgtype': 'm.text',
        'body': 'Hello world',
      });
      expect(
    ...
    test('sendToDeviceEncryptedChunked', () async {
      ...
      for (var i = 0; i < 30; i++) {
        ...
        final signature = account.sign(String.fromCharCodes(canonicalJson.encode(keyObj)));
        keyObj['signatures'] = {
          userId: {
    ...
    test('send to_device queue', () async {
      ...
      expect(
        json.decode(
          FakeMatrixApi.calledEndpoints['/client/v3/sendToDevice/foxies/floof_txnid']?[0],
        )['messages'],
        foxContent,
      );
      expect(
        json.decode(
          FakeMatrixApi.calledEndpoints['/client/v3/sendToDevice/raccoon/raccoon_txnid']?[0],
        )['messages'],
        raccoonContent,
      );
      FakeMatrixApi.calledEndpoints.clear();
      ...
      expect(
        FakeMatrixApi.calledEndpoints['/client/v3/sendToDevice/foxies/floof_txnid'],
        null,
      );
      expect(
        FakeMatrixApi.calledEndpoints['/client/v3/sendToDevice/raccoon/raccoon_txnid'],
        null,
      );
      expect(
        json.decode(
          FakeMatrixApi.calledEndpoints['/client/v3/sendToDevice/bunny/bunny_txnid']?[0],
        )['messages'],
        bunnyContent,
      );
      await client.dispose(closeDatabase: true);
    ...
    test('send to_device queue multiple', () async {
      ...
      await client.sendToDevice('raccoon', 'raccoon_txnid', raccoonContent).catchError((e) => null);
    ...
    test('upload', () async {
      final client = await getClient();
      final response = await client.uploadContent(Uint8List(0), filename: 'file.jpeg');
    ...
    test('object equality', () async {
      ...
      final user1 = User('@user1:example.org', room: Room(id: '!room1', client: matrix));
      final user2 = User('@user2:example.org', room: Room(id: '!room1', client: matrix));
      // receipts
      ...
      // rooms
      expect(
        Room(id: '!room1', client: matrix) == Room(id: '!room1', client: matrix),
        true,
      );
      expect(
        Room(id: '!room1', client: matrix) == Room(id: '!room2', client: matrix),
        false,
      );
      // ignore: unrelated_type_equality_checks
    ...
    test('getEventByPushNotification', () async {
      ...
      Event? event;
      event = await client.getEventByPushNotification(PushNotification(devices: []));
      expect(event, null);
      ...
      final storedEvent = await client.database?.getEventById('123', event!.room);
      ...
      expect(event?.type, 'm.room.message');
      final storedEvent2 =
          await client.database?.getEventById('143273582443PhrSn:example.org', event!.room);
    ...
    test('Rooms and archived rooms getter', () async {
      ...
      expect(
        client.rooms.length,
        3,
        reason: 'Count of invited+joined before loadArchive() rooms does not match',
      );
      expect(
