...
void main() {
  ...
  group('Device keys', tags: 'olm', () {
    ...
    test('set blocked / verified', () async {
      ...
      client.userDeviceKeys[client.userID]?.deviceKeys['UNSIGNEDDEVICE'] = DeviceKeys.fromJson(
        {
          'user_id': '@test:fakeServer.notExisting',
          ...
          'keys': {
            'curve25519:UNSIGNEDDEVICE': 'ds6+bItpDiWyRaT/b0ofoz1R+GCy7YTbORLJI4dmYho',
            'ed25519:UNSIGNEDDEVICE': 'CdDKVf44LO2QlfWopP6VWmqedSrRaf9rhHKvdVyH38w',
          },
          'signatures': {
            '@test:fakeServer.notExisting': {
      ...
      expect(key.encryptToDevice, true);
      expect(
        client.userDeviceKeys[client.userID]?.deviceKeys['UNSIGNEDDEVICE']?.encryptToDevice,
        true,
      );
      ...
      expect(
        FakeMatrixApi.calledEndpoints.keys.any((k) => k == '/client/v3/keys/signatures/upload'),
        true,
      );
    ...
    test('verification based on signatures', () async {
      ...
      expect(
        user.getKey('F9ypFzgbISXCzxQhhSnXMkc1vq12Luna3Nw5rqViOJY')?.crossVerified,
        true,
      );
    ...
    test('start verification', () async {
      var req = await client.userDeviceKeys['@alice:example.com']?.deviceKeys['JLAFKJWSCS']
          ?.startVerification();
