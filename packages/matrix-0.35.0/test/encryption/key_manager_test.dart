...
void main() {
  group('Key Manager', tags: 'olm', () {
    ...
    test('outbound group session', () async {
      ...
      client.userDeviceKeys['@alice:example.com']?.deviceKeys['NEWDEVICE'] = DeviceKeys.fromJson(
        {
      ...
      // force wipe
      sess = await client.encryption!.keyManager.createOutboundGroupSession(roomId);
      await client.encryption!.keyManager.clearOrUseOutboundGroupSession(roomId, wipe: true);
      expect(
        client.encryption!.keyManager.getOutboundGroupSession(roomId) != null,
        false,
      );
    ...
    test('inbound group session', () async {
      ...
      client.encryption!.keyManager.clearInboundGroupSessions();
      expect(
        client.encryption!.keyManager.getInboundGroupSession(roomId, sessionId) != null,
        false,
      );
      ...
      await client.encryption!.keyManager.loadInboundGroupSession(roomId, sessionId);
      expect(
        client.encryption!.keyManager.getInboundGroupSession(roomId, sessionId) != null,
        true,
      );
    ...
    test('Reused deviceID attack', () async {
      ...
      expect(
        client.userDeviceKeys['@alice:example.com']?.deviceKeys['JLAFKJWSCS'] != null,
        true,
      );

      // Alice removes her device
      client.userDeviceKeys['@alice:example.com']?.deviceKeys.remove('JLAFKJWSCS');

      // Alice adds her device with same device ID but different keys
      final oldResp = FakeMatrixApi.currentApi?.api['POST']?['/client/v3/keys/query'](null);
      FakeMatrixApi.currentApi?.api['POST']?['/client/v3/keys/query'] = (_) {
        oldResp['device_keys']['@alice:example.com']['JLAFKJWSCS'] = {
          ...
          'keys': {
            'curve25519:JLAFKJWSCS': 'WbwrNyD7nvtmcLQ0TTuVPFGJq6JznfjrVsjIpmBqvDw',
