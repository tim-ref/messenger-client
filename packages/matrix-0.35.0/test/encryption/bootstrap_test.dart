      ...
      () async {
        ...
        bootstrap = client.encryption!.bootstrap(
          onUpdate: (bootstrap) async {
            ...
            } else if (bootstrap.state == BootstrapState.askSetupOnlineKeyBackup) {
        ...
        for (final keyType in {'master', 'user_signing', 'self_signing'}) {
          final privateKey = base64.decode(await defaultKey.getStored('m.cross_signing.$keyType'));
          final keyObj = olm.PkSigning();
          try {
            final pubKey = keyObj.init_with_seed(privateKey);
            expect(
              pubKey,
              client.userDeviceKeys[client.userID]?.getCrossSigningKey(keyType)?.publicKey,
            );
          } finally {
            keyObj.free();
          }
