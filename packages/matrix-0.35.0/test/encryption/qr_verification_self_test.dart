...
void main() async {
  ...
  Future<void> ingestCorrectReadyEvent(
    ...
    final copyKnownVerificationMethods = List.from(req2.knownVerificationMethods);
    ...
    if (req2.userId == req2.client.userID) {
      if (!(req2.client.userDeviceKeys[req2.client.userID]?.deviceKeys[req2.deviceId]
                  ?.hasValidSignatureChain(verifiedByTheirMasterKey: true) ??
              false) &&
          !(req2.client.userDeviceKeys[req2.client.userID]?.masterKey?.verified ?? false)) {
  ...
  group('Key Verification', tags: 'olm', () {
    ...
    test('Run qr verification mode 1', () async {
      ...
      expect(
        client1.userDeviceKeys[client2.userID]?.deviceKeys[client2.deviceID]?.verified,
        false,
      );
      expect(
        client2.userDeviceKeys[client1.userID]?.deviceKeys[client1.deviceID]?.verified,
        false,
      );
      ...
      client2.userDeviceKeys[client2.userID]!.masterKey!.setDirectVerified(false);
      ...
      expect(
        client2.encryption!.keyVerificationManager.getRequest(req2.transactionId!),
        req2,
      );
    ...
    test('Run qr verification mode 2', () async {
      ...
      // await client1.encryption!.ssss.clearCache();
      final req1 = await client1.userDeviceKeys[client2.userID]!.startVerification(
    ...
    test('Run qr verification mode 1, but fail because incorrect secret', () async {
      // make sure our master key is *not* verified to not triger SSSS for now
      client1.userDeviceKeys[client1.userID]!.masterKey!.setDirectVerified(true);
      client2.userDeviceKeys[client2.userID]!.masterKey!.setDirectVerified(false);
    ...
    test('Run qr verification mode 2, but both unverified master key', () async {
      // make sure our master key is *not* verified to not triger SSSS for now
