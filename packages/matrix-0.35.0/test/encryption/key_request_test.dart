  ...
  group('Key Request', tags: 'olm', () {
    ...
    test('Create Request', () async {
      ...
      for (final entry in FakeMatrixApi.calledEndpoints.entries) {
        ...
        if (entry.key.startsWith('/client/v3/sendToDevice/m.room_key_request') &&
    ...
    test('Reply To Request', () async {
      ...
      session!.allowedAtIndex['@test:fakeServer.notExisting'] = <String, int>{
        matrix.userDeviceKeys['@test:fakeServer.notExisting']!.deviceKeys['OTHERDEVICE']!
            .curve25519Key!: 0,
      };
      event = ToDeviceEvent(
    ...
    test('Receive shared keys', () async {
      ...
      var event = ToDeviceEvent(
        ...
        content: {
          ...
          'sender_claimed_ed25519_key': 'L+4+JCl8MD63dgo8z5Ta+9QAHXiANyOVSfgbHA5d3H8',
        },
        encryptedContent: {
      ...
      await matrix.encryption!.keyManager.handleToDeviceEvent(event);
      expect(
        matrix.encryption!.keyManager.getInboundGroupSession(requestRoom.id, validSessionId) !=
            null,
        true,
      );
