...
EventUpdate getLastSentEvent(KeyVerification req) {
  final entry = FakeMatrixApi.calledEndpoints.entries.firstWhere((p) => p.key.contains('/send/'));
...
void main() async {
  ...
  group('Key Verification', tags: 'olm', () {
    ...
    test('Run emoji / number verification', () async {
      ...
      expect(
        client1.userDeviceKeys[client2.userID]?.deviceKeys[client2.deviceID]?.directVerified,
        true,
      );
      expect(
        client2.userDeviceKeys[client1.userID]?.deviceKeys[client1.deviceID]?.directVerified,
        true,
      );
