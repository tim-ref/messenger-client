...
void main() {
  ...
  setUpAll(() {
    ...
    when(tokenRepo.getFhirToken()).thenAnswer((_) async => defaultOpenIdToken());
  ...
  test('Should correctly deserialize the search result', () async {
    ...
    expect(actualResponse, equals(mockDataString));
  });

  test('Should correctly deserialize the search result for HealthcareService', () async {
    // given
    final mockDataFile = File('test_resources/healthcare_service_test_org_123.json');
    final mockDataString = await mockDataFile.readAsString();
  ...
  test('Should correctly call all pages of a paginated search', () async {
    ...
    final allTitusesPage1String =
        await File('test_resources/all_tituses_fhir_page_1.json').readAsString();
    final allTitusesPage2String =
        await File('test_resources/all_tituses_fhir_page_2.json').readAsString();
    final allTitusesPage3String =
        await File('test_resources/all_tituses_fhir_page_3.json').readAsString();
    ...
    expect(actualResponse, contains("08a678eb-fdc0-4f4d-ac60-d7d8cf3ced90"));
  });
  ...
  test('Should correctly get FHIR visibility', () async {
    ...
    final responseBodyString =
        await File('test_resources/owner_search_with_endpoint.json').readAsString();
