/*
 * TIM-Referenzumgebung
 * Copyright (C) 2024 - akquinet GmbH
 *
 * This program is free software: you can redistribute it and/or modify it under the terms of the GNU Affero General Public License as published by the Free Software Foundation, either version 3 of the License.
 *
 * This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */

import 'package:fluffychat/tim/feature/fhir/dto/coding.dart';
import 'package:fluffychat/tim/feature/fhir/search/fhir_search_result.dart';
import 'package:json_annotation/json_annotation.dart';

part 'practitioner_search_result.g.dart';

@JsonSerializable(createFactory: false)
class PractitionerSearchResult extends FhirSearchResult {
  final String? practitionerName;
  final List<String>? endpointNames;
  final List<String>? endpointAdresses;
  final List<String>? endpointIds;
  final List<Coding>? practitionerQualifications;

  PractitionerSearchResult({
    this.endpointAdresses,
    this.practitionerName,
    this.endpointNames,
    this.endpointIds,
    this.practitionerQualifications,
  });

  @override
  Map<String, dynamic> toJson() => _$PractitionerSearchResultToJson(this);
}
