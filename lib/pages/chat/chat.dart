...
import 'package:fluffychat/tim/feature/testdriver_send_file/filename_dialog.dart';
...
class ChatController extends State<ChatPageWithRoom> {
  ...
  String? get currentMembership {
    ...
    return room.states['m.room.member']?.entries
        .firstWhereOrNull((element) => element.key.contains(userId))
        ?.value
        .content['membership'] as String?;
  }
  ...
  void setMatrixClient(Client c) {
    // first cancel typing with the old matrix client
    if (AppConfig.sendTypingNotifications && currentlyTyping) {
    ...
    // then set the new matrix client
    setState(() => matrixClient = c);
  }
  ...
  void sendFileTestAction() async {
    await showDialog(
      context: context,
      useRootNavigator: false,
      builder: (c) => FilenameDialog(
        room: room,
      ),
    );
  }
