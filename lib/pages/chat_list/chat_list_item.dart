...
class ChatListItem extends StatelessWidget {
  ...
  void clickAction(BuildContext context) async {
    ...
    const mxErrorInvalidRoomCode = "M_UNRECOGNIZED";
    if (room.membership == Membership.invite) {
      final joinResult = await showFutureLoadingDialog(
        ...
        onError: (exception) {
          Logs().e("Failed to open room!", exception);
          try {
            if (mxErrorInvalidRoomCode.equals(exception.errcode)) {
              return L10n.of(context)!.noMatrixRoomFound;
            }
          } catch (_) {}
      ...
      if (joinResult.error != null) {
        try {
          if (mxErrorInvalidRoomCode.equals(joinResult.error.errcode)) {
            await room.leave();
          }
        } catch (_) {}
        return;
