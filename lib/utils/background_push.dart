...
class BackgroundPush {
  ...
  BackgroundPush._(this.client) {
    ...
    firebase.onTokenRefresh.listen((token) {
      Logs().v('New Push Token', token);
      _fcmToken = token;
    });
    FirebaseMessaging.onMessage.listen((message) {
      final data = _addDummyDevices(message.data);
      pushHelper(
  ...
  static Map<String, dynamic>_addDummyDevices(Map<String, dynamic> data) {
    ...
    messageDataWithDefaultDevice['devices'] = [
      {
        'app_id': 'dummyAppId',
        'pushkey': 'dummyPushkey',
      }
    ];
    return messageDataWithDefaultDevice;
  }
  ...
  Future<void> _newUpEndpoint(String newEndpoint, String i) async {
    ...
    var endpoint = AppConfig.pushNotificationsGatewayUrl;
    try {
